...
export async function adminSetProposalStateAction(formData: FormData) {
  ...
  const transaction = await withAdminTransaction('proposal_state', async (client, adminId) => {
    ...
    await recordAdminChange(client, adminId, 'proposal', proposalId.data, 'admin_proposal_state_changed', reason.data.reasonCode, reason.data.reasonText, withoutImageData(current.rows[0]), withoutImageData(updated.rows[0]));
