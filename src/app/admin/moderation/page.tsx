...
export default async function AdminModerationPage() {
  ...
  if (isSupabaseConfigured()) { const supabase = await createSupabaseServerClient(); const { data } = await supabase.from('reports').select('id, proposal_id, category, details, created_at, resolved_at').order('created_at', { ascending: false }); reports = (data ?? []) as typeof reports; }
