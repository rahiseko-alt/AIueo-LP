...
export default async function AdminModerationPage({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  await requireAdmin();
  const { error } = await searchParams; let reports: Array<{ id: string; proposal_id: string; category: string; details: string; created_at: string; resolved_at: string | null }> = [];
