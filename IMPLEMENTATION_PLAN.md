...
| P29 | Googleの自前鍵への切替が効いていない(確定) | P28の操作検証 | 2026-09-09の引継ぎは「Neon ConsoleのGoogleプロバイダーを自前の鍵へ切替済み」「Googleの画面に`AIueo`と表示されることを確認」としていたが、**いずれも事実ではなかった**（失敗記録 F-02・F-07） | **原因側は確定。対処は未実施。** 根拠2点。(1) 本番を実ブラウザで操作すると、Googleの画面は「to continue to `neon.tech`」と表示される。(2) ユーザーのGoogleアカウントに届いた通知メール（2026-09-10 07:21 JST、`noreply-accounts@google.com`）の件名が「Google アカウントの一部のデータを **neon.tech** と共有しました」であり、**Google自身の記録が使用アプリを`neon.tech`だと言っている**。使用中の`client_id`は`1063997916405-quq0arh4eauiuv3rh0d071sigc5dhmj7.apps.googleusercontent.com`。参考として2026-09-07には同じアカウントが「**無題のプロジェクト**」へ許可を出した記録があり、ユーザー自身のクライアント（アプリ名未設定）は別に存在する。つまり本番はユーザーのクライアントではなくNeon側のクライアントを使っている。**切替は Neon Console の画面操作でしか行えず、この環境にNeon/Vercelの認証情報が無いため実行できない**（`env`にトークン無し、`vercel`/`neonctl` CLI無し、`CLOUDSDK_AUTH_ACCESS_TOKEN`はユーザーのものではなく401、Neon Authにプロバイダー設定を読む公開エンドポイントも無いことをすべて実測で確認した）。切替後の検証は`scripts/verify-oauth-entry.mjs --expect-client-id <新しい値>`で機械的に行う |
