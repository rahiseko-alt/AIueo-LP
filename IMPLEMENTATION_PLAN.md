...
| P28 | Google認証で500エラー(新規・未解決) | P27 | P27修正の反映後、ユーザーが実際にGoogle認証を試みたところGoogle側の汎用エラーページ「500. That's an error.」が出たと報告。発生箇所を特定する前にユーザーがセッションを中断し詳細未回答のまま | 切り分け済み・原因未特定。2026-09-10にユーザー指示（コードを読まず実ブラウザ操作とスクリーンショットのみで検証）に従い本番を操作した。入口（`/register`→`accounts.google.com`のログイン画面）と失敗時の戻り（`error=access_denied`／`code`無効→`/register?auth_error=1`の日本語エラー）はいずれも到達を確認し、500は再現しなかった。4xx/5xxの応答は1件も観測されない。**未検証区間はGoogleアカウント選択→同意→トークン交換→`/member/profile`の1区間のみで、500はここで起きている。** 次にユーザーへ聞くのは「500のとき、アカウント選択（メールアドレス入力）は済んでいたか」の1問に絞られた |
| P29 | Googleのログイン画面が`AIueo`ではなく`neon.tech`と表示される(新規・未解決) | P28の操作検証 | 2026-09-09の引継ぎは「自前の鍵に切替済み。Googleの画面に`AIueo`と表示されることを確認してほしい」としていたが、本番の実操作では「to continue to `neon.tech`」と表示される。使用中の`client_id`は`1063997916405-quq0arh4eauiuv3rh0d071sigc5dhmj7.apps.googleusercontent.com` | 未着手。(a)本番がまだNeonの共用クライアントを使っている、(b)自前クライアントのアプリ名が未設定でリダイレクト先ドメインへ落ちている、のいずれか。**この環境からは判別できない【曖昧】**。ユーザーがGoogle CloudのOAuthクライアント画面とNeon Consoleのプロバイダー画面を見て、client_idが一致するかを確認する必要がある。P28の500と同じ設定に起因する可能性がある |
