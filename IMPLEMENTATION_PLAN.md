...
| P25 | 権限別の操作シナリオ作成（P7の前提） | P24 | 権限6区分について、オーソドックスな操作50件と致命的な危険操作50件を、現行コードのガード条件に紐づけて列挙する。各行に「テスト・実測で確認済み／コードにガードはあるが実DB未検証／ガードが無く現状成立する」の判定を付け、公開前に扱いを決めるべき項目を洗い出す | 完了（ブランチ`claude/checkin-6hrtds`、本番マージ待ち。`docs/OPERATION_SCENARIOS.md`。**新たに7件の未対応問題を特定。うち2件は深刻度が高い（管理者の自己停止によるロックアウト、開催状況ボタンの誤操作からの復帰不能）。仕分けはユーザー判断待ち**） |
