...
| P28 | Google認証で500エラー(新規・未解決) | P27 | P27修正の反映後、ユーザーが実際にGoogle認証を試みたところGoogle側の汎用エラーページ「500. That's an error.」が出たと報告。発生箇所を特定する前にユーザーがセッションを中断し詳細未回答のまま | 未着手。次セッションで①どの操作の直後か（`/register`のボタン押下直後かGoogle同意後の戻りか）②表示時のURL（`accounts.google.com`系か`aiueo-lp.vercel.app`系か）をユーザーに確認してから対応する。詳細は`HANDOFF.md`「次にやること」参照 |
