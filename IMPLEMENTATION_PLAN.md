...
| P34 | 通報の階層下げ・画像添付・立場の統一・導線の実操作検証 | P33 | ユーザーの指示4件。①通報の操作画面が大きく通報ページに見えるので1階層下げる、②画像を添付できるようにする、③権限の曖昧を①管理者②登録者③一般へ統一する、④登録者ページと企画ページの一貫性を、操作経路のチェックリストで順方向・分岐・逆方向とも実操作で検証する | **未検証区間: 会員側の画面（`/member` 配下）の導線が本番でも期待どおりかは未実測（理由: この環境に実Googleアカウントの資格情報が無く、未ログインでは `/member/profile` へ転送されて外から見えない。ローカルの検証装置では40項目すべてOK）。確かめる手段: ユーザーが本番でログイン中に `/member/proposals` を開き、上部に「会員ページ / 自分の企画」と出ること、企画を開くと「公開ページを見る」が出ることを見る。** **②画像は端から端まで本番で通った**（2026-09-12。ユーザーが写真を添付し、配信された実物をこちらで取得して確認: HTTP 200 / `image/jpeg` / 89,357バイト / **1280×853px**＝設計どおり長辺1280へ縮小。企画詳細・一覧・トップのいずれにも表示。これによりログイン・企画編集画面・保存の経路も本番で動いていることが確かめられた）。 PR #59（`22c8951`）で`main`へマージ・本番反映済み。`drizzle/0003`も本番適用済みで、**適用されたことをこちらで機械的に確かめた**（トップ200・企画一覧200・`/api/proposals/<uuid>/image` が404＝列が存在する。500なら列が無い）。**ログインの要らない範囲は本番でも実測した**: 企画詳細の通報入力欄0個（修正前は2個）、「管理者へ連絡」ボタン1個、通報ページ200・入力欄2個・戻り導線あり・noindexあり、企画詳細からトップと会員登録へ到達可、`/register` に3つの立場が表示。 ④の実測で、**修正前は25項目中12項目が到達不能**だった（公開中の自分の企画の公開ページを見る手段が無い、企画編集から来た道へ戻れない、企画詳細からトップ・会員登録へ行けない、右上バッジが `/member/profile` 固定、ほか）。①は `/events/[slug]/report` を新設し詳細ページからは「管理者へ連絡」ボタンだけにした。②は `drizzle/0003_proposal_images.sql` でDBへ保存し、送信前にブラウザ側で長辺1280pxへ縮める（実測 2314KB→22KB）。下書きの画像は持ち主と管理者以外に見せない。③は `src/lib/auth/roles.ts` に3つの立場を1か所で定め、`/register` と `/member` へ同じ表を出した。④はパンくずと双方向の導線で12件すべて解消。修正後は操作経路40項目・画像10項目がすべてOK。検証は `bash scripts/local-verify-rig.sh` で1コマンドで再現できる |
| P35 | 応募者を主催者のフォームへ渡す導線 | P34 | ユーザーの質問「応募が来た時に、応募者を管理する機能はあるか？」への回答が「無い（仕様どおり）」だったため、**主催者が自分で用意した申し込みフォームへ1押しで行ける導線**を足す。AIueoは応募者の氏名・連絡先を保存しない（`MEMBERSHIP_FEATURE_SPEC.md` 実装前提を変えない） | **未検証区間: 本番で実ログインした状態からのURL入力（理由: この環境に実Googleアカウントの資格情報が無い。確かめる手段: ユーザーが本番で企画を開き、申し込みフォームのURLを貼って保存し、公開ページに「参加を申し込む」ボタンが出ることを見る）。** PR #62（`48e7edc`）で`main`へマージ・本番反映済み。`drizzle/0004`も本番適用済みで、**適用をこちらで機械的に確かめた**（`48e7edc`のデプロイ成功＋企画詳細200＝列が存在する。列が無ければ500）。 `drizzle/0004_proposal_application_url.sql` で `application_url` 列を足し、DB側のCHECK制約で `http(s)://` 以外と2000字超を拒む。アプリ側は `src/lib/proposals/application-url.ts` で同じ条件を検査し、理由を画面へ出す。公開ページのボタンは別タブで開き `rel="noopener noreferrer nofollow"` を付ける。企画者の編集画面と管理者の編集画面の両方に欄を置いた（G1-06「管理者は全項目を編集できる」）。実ブラウザで10項目を実測（肯定・否定・危険URLの3方向） |
| P36 | 管理者操作の監査記録に画像の中身が入る不具合（P34の対策漏れ） | P34 | `recordAdminChange` に企画の行がそのまま渡っており、`image_data` が `{"data":[...],"type":"Buffer"}` として `moderation_actions` と `audit_log` の計4か所へ書かれていた。P34で入れた `withoutImageData` は `proposal_versions` にしか掛かっていなかった | 完了（実測で確定・修正・再実測）。**89KBの画像が付いた企画に管理者が1操作するたび 1,433,122 バイト**が積まれることを、本番と同じマイグレーションを当てたPostgreSQL 16で実測した。`withoutImageData` を通す修正後、同じ操作で **815 バイト**（440分の1）になり、画像そのものは消えていないことも実測した |
| P37 | 企画の一覧に参加人数を出す | P35 | ユーザーの指示「参加者を企画の表に表示する機能つけろ。企画の中に入る前」。出せる情報がDBに1つも無かったため3案を提示し、**「人数だけ（企画者が入力）」**が選ばれた。応募者の氏名・連絡先はAIueoに保存しない（`MEMBERSHIP_FEATURE_SPEC.md` 実装前提のまま） | **未検証区間: 本番で実ログインした状態からの人数入力（理由: この環境に実Googleアカウントの資格情報が無い。確かめる手段: ユーザーが本番で企画に定員と人数を入れて保存し、`/events` のカードに「参加人数 3 / 10人（残り7）」と出ることを見る）。あわせて `drizzle/0005_proposal_headcount.sql` は本番Neonへ未適用で、マージより先に適用が要る。** `capacity`（未設定＝上限なし）と `participant_count`（既定0）を足し、DB側のCHECK制約とアプリ側の判定（`src/lib/proposals/headcount.ts`）で二重に守る。表示は企画の一覧・トップページのカード・企画詳細の3か所。定員を超える人数は拒まず「満席」と出す（主催者が実際に多く受け入れることがあるため）。企画者と管理者の両方の編集画面に欄を置いた（G1-06）。**言葉は「参加人数」で統一し、主催者を含めた数とした**（1なら主催者だけ、2なら主催者ともう1人。「参加者」だと主催者だけなのか別にいるのか読み手に分からないため。ユーザーの指示）。実ブラウザで13項目を実測（肯定・否定・おかしな値の3方向） |
...
| 2026-09-12 | P34の画像機能が、本番で端から端まで通った。ユーザーが本番でログインし自分の企画へ写真を1枚添付した。**報告だけを根拠にせず、配信された実物をこちらで取得して確かめた** | 画像の未検証区間は解消。残るのは会員側の画面の導線が本番でも期待どおりかのみ | **実測（本番・未ログインで取得）**: HTTP 200 / `content-type: image/jpeg` / `content-length: 89357`（実バイト数も一致）/ JPEGのSOFセグメントから読んだ大きさが **1280×853px**（設計どおり長辺1280へ縮小されている）/ `cache-control: public, max-age=60`。企画詳細ページ・企画一覧・トップページのいずれにも表示。**画像が保存できた以上、ログイン・企画編集画面・保存の経路も本番で動いている** |
| 2026-09-13 | P36: `src/app/admin/actions.ts` の `recordAdminChange` へ渡す企画の行を `withoutImageData` に通した（94行目・182行目）。P34で入れた対策が `proposal_versions` にしか掛かっておらず、管理者が状態変更・内容編集をするたびに画像の中身が監査記録へ二重に積まれていた | 完了 | **実測（PostgreSQL 16、本番と同じ4マイグレーション）**: 89,357バイトの画像を付けた企画に管理者の状態変更を1回行い、`moderation_actions` の before/after が各 358,293 / 358,268 バイト、`audit_log` も同じで**1操作あたり合計 1,433,122 バイト**。修正後は `moderation_actions` が 815 / 818 バイト、`image_data` キーは無く `has_image: true` が入り、企画の画像は 89,357 バイトのまま残る |
| 2026-09-13 | P35/P36の本番反映: PR #62（`48e7edc`）を`main`へマージし、本番へ反映した。ユーザーが `drizzle/0004_proposal_application_url.sql` を本番Neonへ適用した。**適用されたことは、ユーザーの報告ではなくこちらで機械的に確かめた**（失敗記録 F-02 への対処）。あわせて、PR作成直後にCIが1度も起動しなかったこと（空コミット・閉じ直しは禁止なので行わず、枝分かれ解消の押し直しで正常起動）と、squashマージ由来の枝分かれによる競合を台帳へ記録した | P35・P36とも本番反映済み。残る未検証区間は、本番で実ログインしてURLを貼るところだけ | **実測（本番）**: `48e7edc` のVercelデプロイが `success`（新しいコードが動いている）。企画一覧 200。**企画詳細（公開中2件とも）200** — このページは `application_url` を select するので、列が無ければ500になる。トップ 200。「参加を申し込む」ボタンは未表示（まだ誰もURLを入れていないので正しい）。**マージ前に確かめる手段は無かった**（プレビューはVercelのSSO保護で302、本番の旧コードは当該列を読まない）ため、その旨をユーザーへ先に伝えたうえでマージした |
| 2026-09-13 | P35: 企画に「参加申し込みフォームのURL（任意）」を足した。ユーザーの質問「応募が来た時に、応募者を管理する機能はあるか？」への回答が「無い（AIueoは当事者にならない仕様のため）」だったのを受け、**応募者データをAIueoに持たせないまま、主催者のフォームへ1押しで行ける**ようにしたもの。`drizzle/0004_proposal_application_url.sql`（1つの`do`ブロック・何度流しても同じ）、`src/lib/proposals/application-url.ts`（`http(s)://` 以外・2000字超・空白を拒む）、企画フォームと管理者の編集フォームの欄、公開ページの「参加を申し込む（外部サイトが開きます）」ボタン | 未検証区間: 本番で実ログイン状態からのURL入力 | **実測（実ブラウザ・10項目すべてOK）**: 欄がある／AIueoに届かないと画面に書いてある／DBへ保存される／公開ページにボタンが出る／`target=_blank` と `rel=noopener noreferrer nofollow` が付く／外部サイトへ移ると書いてある／**押すと実際に別タブでそのURLが開く**／`javascript:alert(document.cookie)` は保存されずDBの値も変わらない／弾いた理由が画面に出る／空欄に戻すとボタンが消える。**否定側**: 公開ページからボタンの区画を外すと A-04 がNGになり全体NGになることを実測。**DB側**も `javascript:` 拒否・`https://` 受理・2000字超拒否を実測。再現手段は `bash scripts/local-verify-rig.sh`（40＋10＋10項目） |
| 2026-09-14 | P37: 企画の一覧カードに参加人数を出せるようにした。`drizzle/0005_proposal_headcount.sql`（`capacity`・`participant_count`、1つの`do`ブロック）、`src/lib/proposals/headcount.ts`（判定と表示文言を1か所へ）、企画者と管理者の編集欄、`/events`・トップ・企画詳細の3か所への表示。**応募者の個人情報はAIueoに保存しない**（仕様は変えていない）。あわせて、新しい2欄の日本語名の登録漏れ（画面に英語の`capacity`と出る）と、必須欄の通知文言が範囲外の値のとき実態と食い違う点を直した | 未検証区間: 本番で実ログイン状態からの人数入力。`drizzle/0005`は本番未適用 | **実測（実ブラウザ13項目すべてOK）**: 欄がある／説明が書いてある／DBへ保存される／**企画の一覧カードとトップページに「参加人数 3 / 10人（残り7）」が出る**／企画詳細にも同じ数が出る／定員まで埋めると「満席」／定員が空欄なら「参加人数 4人」／どちらも空なら行自体が出ない／定員0は保存されず日本語で「定員」と出る／DB側のCHECKが0・マイナス・上限超えを拒否／サーバー側の判定を実モジュールへ直接渡して全角・小数・マイナス・文字・0・上限超えの拒否と正しい値の通過を確認。**否定側**: 一覧から表示を外すと H-04・H-07・H-08 がNGになり全体NG。操作経路40・画像10・申し込みURL10に回帰なし。`lint` 0件、`typecheck` 0件、build成功、Playwright 全114件緑 |
