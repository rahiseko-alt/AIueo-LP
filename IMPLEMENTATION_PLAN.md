...
| 2026-09-14 | P37: 企画の一覧カードに参加人数を出せるようにした。`drizzle/0005_proposal_headcount.sql`（`capacity`・`participant_count`、1つの`do`ブロック）、`src/lib/proposals/headcount.ts`（判定と表示文言を1か所へ）、企画者と管理者の編集欄、`/events`・トップ・企画詳細の3か所への表示。**応募者の個人情報はAIueoに保存しない**（仕様は変えていない）。あわせて、新しい2欄の日本語名の登録漏れ（画面に英語の`capacity`と出る）と、必須欄の通知文言が範囲外の値のとき実態と食い違う点を直した | 未検証区間: 本番で実ログイン状態からの人数入力。`drizzle/0005`は本番未適用 | **実測（実ブラウザ12項目すべてOK）**: 欄がある／説明が書いてある／DBへ保存される／**企画の一覧カードとトップページに「参加 3 / 10人（残り7）」が出る**／企画詳細にも同じ数が出る／定員まで埋めると「満席」／定員が空欄なら「参加 4人」／どちらも空なら行自体が出ない／定員0は保存されず日本語で「定員」と出る／DB側のCHECKが0・マイナス・上限超えを拒否／サーバー側の判定を実モジュールへ直接渡して全角・小数・マイナス・文字・0・上限超えの拒否と正しい値の通過を確認。**否定側**: 一覧から表示を外すと H-04・H-07・H-08 がNGになり全体NG。操作経路40・画像10・申し込みURL10に回帰なし。`lint` 0件、`typecheck` 0件、build成功、Playwright 全114件緑 |
