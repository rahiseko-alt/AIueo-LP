...
| P25 | 権限別の操作シナリオ作成（P7の前提） | P24 | 権限6区分について、オーソドックスな操作50件と致命的な危険操作50件を、実施できる手順書として作成する。**シナリオの作成までが範囲で、実施はしない**（実施するのは人。エージェントには検証させない方針） | 作成完了（PR #40、`8bccffe`で`main`へマージ済み。`docs/OPERATION_SCENARIOS.md`）。**実施は未着手**。運営アカウント・会員アカウント・テスト用DBが揃っていないため |
