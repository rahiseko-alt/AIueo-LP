...
- **P32（2026-09-11）: 画面に出ていた英語の状態名（`draft`/`published`/`planning`等）を日本語へ統一した。** 表示用の対応表を`src/lib/proposals/labels.ts`へ1か所に集め、言葉は操作ボタンと揃えた（公開中／下書き／開催決定／参加者満席／終了／中止など）。会員3画面・管理3画面に適用。**未検証区間: 本番で実ログイン状態での表示（理由: この環境に実Googleアカウントの資格情報が無い。確かめる手段: ユーザーが本番の企画一覧を開き「公開中」「下書き」と出ることを見る）**
- **P31（2026-09-11）: 公開済み企画を下書きに戻す機能を追加した。** 元から`updateProposalAction`に`intent=draft`の経路はあったが、ボタンの文言が「下書き保存」のままで押すと公開が落ちることが画面に書かれておらず、しかも編集フォームの3つの掲載確認チェックを入れ直さないと送信できないため、事実上使えなかった（実測）。単独の`unpublishProposalAction`と「公開をやめる」ボタンを追加し、公開中はフォームのボタン文言も「下書きとして保存（公開はやめます）」「公開したまま保存」に変えた。**未検証区間: 本番で実ログイン状態からの取り下げ（理由: この環境に実Googleアカウントの資格情報が無い。確かめる手段: ユーザーが本番で「公開をやめる」を押し、`/events`から消えることを見る）**
- **P30（2026-09-10）: 「保存するを押しても何も進まない」の原因を特定して修正した。** ブラウザ標準の必須チェックが送信を黙って止めており、PC幅では画面のスクロール位置すら動かないことを実ブラウザで実測した。空の欄の名前を日本語で画面に出す`src/components/required-fields-notice.tsx`を新設し、企画フォームと会員登録フォームへ入れた。**未検証区間: 本番で実ログイン状態からの保存（理由: この環境に実Googleアカウントの資格情報が無い。確かめる手段: ユーザーが本番で金銭欄を空のまま「下書き保存」を押し、赤い枠が出るか見る）**
- **P28（Google認証500エラー）: 2026-09-10 その4のセッションで、実ブラウザ操作による経路の切り分けを実施した。** 入口（`/register`→Googleのログイン画面）と失敗時の戻り（キャンセル・コード無効→`/register?auth_error=1`の日本語エラー）は**操作で到達を確認済み**。500は再現しなかった。未検証区間は**Googleアカウント選択→同意→トークン交換→`/member/profile`**の1区間だけで、ユーザーが見た500はこの区間で起きている。詳細と証跡は本ファイル「今回の作業（2026-09-10 その4）」を参照。**2026-09-10 その6のセッションで、ユーザーから「解消した」と報告があった**（PR #52・#53の反映後。ただしこれはユーザーの報告であって、こちらで500の消失を機械的に確かめたわけではない【曖昧】。原因も未特定のままである）
...
- 品質ゲート: `lint` / `typecheck` / `build` / Playwright 111件が GitHub Actions で PR ごとに必須実行され、緑
...
## 今回の作業（2026-09-11 その3 / P33 引継ぎ到達確認の誤警報を止める、Claude Code on the web）

ユーザーの指示は「（未マージ3ブランチを）削除しろ」。**削除はできなかった。**
`git push --delete` が3回とも `HTTP 403` で拒否された（このセッションのGitHub権限は
プッシュを許すがブランチ削除を許さない）。実測した事実であり、権限が変わらない限り
こちらからは消せない。

そこでユーザーの指示により、**警告を出す側を直した**。

### 直したもの

`.claude/hooks/session-start.sh` の引継ぎ到達確認に**条件C**を足した。
判定は元から2条件の論理積だったが、squash マージ済みで内容が `main` にあるブランチは
どちらも満たしてしまい、毎セッション警告に出ていた。**本物の未到達が埋もれる。**

- **`.claude/handoff-verified.txt`**（新設）に、突き合わせ済みブランチの
  **先端のコミットSHA**を1行ずつ書く。フックはこのSHAに一致するブランチを落とす。
- **ブランチ名ではなくSHAを鍵にした。** そのブランチへ新しいコミットが積まれれば
  SHAが変わり、警告は復活する。「一度無視したら永久に無視」にならない。
- **落とした件数は毎回画面に出す。**（「3 件は突き合わせ済みとして除外」）
  黙って消すと、登録が間違っていても誰も気付けない。
...
| 見たこと | 実測 |
| --- | --- |
| 3件を登録した状態 | 「✅ 未到達のブランチは無い」＋「3 件は突き合わせ済みとして除外」 |
| **否定側**: 1件をリストから外す | その1件だけが警告に戻った（`claude/checkin-6hrtds`） |
| **肯定側**: 本物の未到達ブランチを1件作る | **登録済み3件は落ちたまま、作った1件だけが警告に出た** |

肯定側の検証用ブランチは、作業ツリーに触れず `commit-tree` でrefだけ作り、確認後に削除した。

品質ゲート: `lint` 0件、`typecheck` 0件、`NEXT_PUBLIC_NEON_AUTH_ENABLED=true build` 成功、
Playwright 全111件緑（bashファイルのみの変更で、アプリへの影響は無い）。

### 途中で起きた自分のミスと、その訂正

検証用ブランチを作る際に `git checkout -B` と `git commit -am` を使い、**作業中だった
フックの修正を検証用のコミットへ巻き込んで消してしまった**。そのまま出した「肯定側」の
結果は、条件Cが入っていない状態の出力で、検証として無効だった。気付いて修正を再適用し、
作業ツリーに触れない方法（`commit-tree` でrefだけ作る）で取り直した。上の表は取り直した
後の値である。

## 今回の作業（2026-09-11 その2 / P32 状態表示を日本語へ統一、Claude Code on the web）

ユーザーの指示は「ドラフトとかpublicとかplanningじゃ分かりにくい、公開中など他のボタンと
同じ表示で統一しろ」。

### 直したもの

DBに入っている英語の状態名（`draft` / `published` / `planning` 等）が、そのまま画面へ
出ていた。表示用の日本語を `src/lib/proposals/labels.ts` に1か所へ集め、
**言葉は操作ボタンと同じにした**（「開催決定」「参加者満席」「終了」「中止」は
企画ページのボタンと同じ文字）。
...
| `expired` | 公開期限切れ |
| `hidden` | 管理者により非公開 |
| `ended` / `cancelled` | 終了 / 中止 |
| `planning` | 調整中 |
| `confirmed` / `full` / `completed` / `cancelled` | 開催決定 / 参加者満席 / 終了 / 中止 |
| `public` / `unlisted` | 公開 / 限定公開 |
| `pending_profile` / `active` / `suspended` / `withdrawn` | 登録の途中 / 有効 / 利用停止 / 退会済み |
| `member` / `admin` | 会員 / 管理者 |

直した画面は6つ。`/member/proposals`（一覧）、`/member/proposals/[id]`（詳細のバッジ）、
`/member/history`（履歴）、`/admin/proposals`（一覧）、`/admin/proposals/[id]`（状態の選択肢）、
`/admin/members`（権限・状態と、会員状態の選択肢）。

`/member/proposals/[id]` にあった `LOCKED_STATUS_LABELS`（`hidden`/`ended`/`cancelled` の
日本語を別に持っていた）は、共通の対応表を参照する形へ置き換えた。画面ごとに別の
言い回しを書かないためである。

**対応表に無い値はそのまま英語で出す。** 状態が増えたときに空欄になるより、
英語のままでも出ているほうが気づける。

### 検証（実ブラウザ）

ローカルにPostgreSQL 16を立てて本番と同じ3migrationを適用し、
`published` / `draft` / `hidden` / `auto_hidden` / `ended` の5件を作って実Chromiumで見た。

- 企画一覧・企画詳細（4状態）・履歴の6画面で、**期待した日本語がすべて出ている**こと、
  および**英語の状態名が画面の文字として1つも残っていない**ことを実測した（exit 0）。
- 管理画面は権限を `admin` にして確認した。`/admin/proposals` の一覧、
  `/admin/proposals/[id]` の状態の選択肢（下書き／修正のお願いあり／管理者により非公開／
  終了／中止）、`/admin/members` の「会員・管理者」表示と選択肢（有効に戻す／利用停止／
  登録取消）が日本語で出ることを確認した。
- **否定側**: 一覧の1か所を元の `{proposal.status} · {proposal.event_status}` に戻すと、
  同じ検証が「出ていない」5件と「英語の残り」6件を挙げて落ちた。この置き換えが
  効いていることの実測である。
...
- 同意履歴に出る規約の版番号 `draft-2026-08-31` は、状態名ではなく**同意した規約の版の
  記録**である。書き換えると同意の証跡が実態とずれるため触っていない。読みにくさは残る。
- 管理画面の「開催形式」「金銭種類」「金銭条件JSON」の入力欄は、管理者がDBの値を
  直接書き換えるための欄なので英語のままにした。

### 未検証区間

**本番で、実際にGoogleログインした状態での表示は未実測である。**
理由: この環境に実Googleアカウントの資格情報が無く、ログインできない。
確かめる手段: ユーザーが本番の企画一覧を開き、「公開中」「下書き」と出ることを見る。

## 今回の作業（2026-09-11 / P31 公開済み企画を下書きに戻す、Claude Code on the web）

ユーザーの指示は「公開済み企画を下書きに戻す という機能を追加しろ」。

### 着手前に分かった実態（前セッションの説明の訂正）

`updateProposalAction` は `intent=draft` を受けると `published` から `draft` へ戻す処理を
元から持っていた。**しかし利用者からは事実上使えなかった。** 理由は2つある。

1. ボタンの文言が「下書き保存」のままで、**押すと公開が取り下がることが画面のどこにも
   書いていない**。取り下げたい人はこのボタンに辿り着けず、取り下げたくない人は
   気づかず公開を落とす。
2. 編集フォームの3つの掲載確認チェックは、編集画面では常に外れた状態で出る。
   ブラウザの必須チェックに引っかかるため、**取り下げるだけなのに掲載の確認3つを
   入れ直さないと送信できない**。実際にローカルで押して、送信が止まることを実測した。

前のセッションで「下書きへ戻す、を行えます」とユーザーへ説明したが、上のとおり
実際には通らなかった。訂正する。

### 追加したもの

- **`unpublishProposalAction`**（`src/app/member/proposals/[id]/actions.ts`）。
  公開中の自分の企画を `draft` へ戻すだけの単独のServer Action。所有者チェック
  （`for update` ＋ UPDATE文にも `owner_id`）と、`status = 'published'` のときだけ
  実行する条件を持つ。管理者の緊急非公開（`hidden`）・終了・中止は対象にしない
  （下書きへ戻すと措置を打ち消すことになるため）。`proposal_versions` と
  `audit_log` に `organizer_unpublish` として記録する。
- **「公開をやめる」の区画**（`/member/proposals/[id]`）。公開中のときだけ出る。
  ボタン1つで、フォームの入力もチェックも要らない。内容と `published_at`、
  管理者とのメッセージは残る。
- **編集フォームのボタンの文言を、公開中かどうかで変える**。公開中は
  「下書きとして保存（公開はやめます）」と「公開したまま保存」になり、押した結果が
  読んで分かるようにした。公開していないときは従来どおり「下書き保存」「公開する」。

### 検証（実ブラウザ。肯定・否定・改ざんの3方向）

ローカルにPostgreSQL 16を立てて本番と同じ3migrationを適用し、公開中の企画を作って
実Chromiumで操作した。

| 見たこと | 実測 |
| --- | --- |
| 公開中の画面 | 「公開をやめる」ボタンが1つ、説明文あり。フォームは「下書きとして保存（公開はやめます）」「公開したまま保存」 |
| ボタンを押す | DBの `status` が `published` → `draft`。`/events` の一覧から消えた |
| 押した後の画面 | 「公開をやめる」は消え、フォームは「下書き保存」「公開する」に戻る |
| もう一度「公開する」 | `published` に戻り、`/events` にも再び出る |

**改ざん**: 自分の画面の隠し項目を他人の企画IDへ書き換えて送信した。
他人の企画は `published` のまま変わらなかった（所有者チェックが効いている）。

**否定側**: 追加した「公開をやめる」の区画を外して同じ検証を流すと、ボタンが見つからず
exit 1 になった。**この追加が効いていることの実測である。**
...
**本番で、実際にGoogleログインした状態から「公開をやめる」を押すまでは未実測である。**
理由: この環境に実Googleアカウントの資格情報が無く、ログインできない。
確かめる手段: ユーザーが本番で自分の公開中の企画を開き、「公開をやめる」を押して、
`https://aiueo-lp.vercel.app/events` からその企画が消えることを見る。

## 今回の作業（2026-09-10 その6 / P30 「保存を押しても何も進まない」の原因特定と修正、Claude Code on the web）

ユーザーの報告は「（Google認証の500は）解消した。しかし保存するを押した後に何も進まない」。

### 原因（実ブラウザで再現して特定した。推測ではない）

**ブラウザ標準の必須チェックが送信を止めていた。** 止めたことが利用者に伝わらない。

必須欄が空のまま送信ボタンを押すと、ブラウザは送信を中止し、その欄に小さな吹き出しを出す。
吹き出しは数秒で消え、押したボタンから離れた位置に出る。**画面のスクロール位置すら動かない**
ので、利用者からは「押しても何も起きない」としか見えない。

ローカルにPostgreSQL 16を立てて本番と同じ3つのmigrationを適用し、開発サーバーへ
実際のChromiumで入って再現した。実測値は次のとおり。

| 再現した状況 | 実測結果 |
| --- | --- |
| 金銭欄（金銭条件の説明・精算方法）が空、PC幅1280px | 送信されない。**スクロール位置 1238 → 1238（1pxも動かない）** |
| 同、スマホ幅390px | 同じく送信されない |
| 開催候補日時を「日付だけ選んで時刻を入れていない」 | 送信されない。コンソールに `The specified value "2026-09-20" does not conform to the required format` |
| 3つの掲載確認チェックの入れ忘れ | 送信されない |

金銭欄はページのかなり下にあり、しかも「なし」でも入力が要る。**空にしたまま気づかず
ボタンを押す形が、いちばん起きやすい。**

### 修正

`src/components/required-fields-notice.tsx` を新設した。送信ボタンを押した時点で、
ブラウザと同じ判定をこちらでも行い、**空の欄の名前を日本語で、消えない形で画面に出す**。
同時に最初の空欄まで画面を送り、そこへカーソルを置く。ブラウザ標準の動きは止めていない。

    次の項目が空のため、まだ保存していません。入力してから、もう一度ボタンを押してください。
    金銭条件の説明 / 精算方法

日付欄は「日付だけ入れて時刻が空」でも不正になるが、見た目には入って見える。この欄だけは
「開催候補日時（日付と時刻の両方）」と添える。

同じ仕組みを2つのフォームへ入れた。

- `src/components/proposal-form.tsx`（企画登録・編集。「下書き保存」「公開する」）
- `src/components/profile-completion-form.tsx`（会員登録。「同意して会員登録を完了する」）

### 検証（肯定・否定の両方を実測した）

肯定側: 実ブラウザで3通り動かした。

    ===== 金銭欄が空-PC (1280px) =====   保存された: false（期待どおり）
    画面のメッセージ: 「…金銭条件の説明 / 精算方法」
    ===== 金銭欄が空-スマホ (390px) ===== 同上
    ===== 全部入力 (1280px) =====        保存された: true（期待どおり）
    判定: OK  EXIT=0

否定側: 追加した `onClick` と通知の表示を外して同じ検証を流すと **判定: NG / EXIT=1** になった。
**この修正が効いていることの実測である。**

品質ゲート: `npm run lint` 0件、`npm run typecheck` 0件、
`NEXT_PUBLIC_NEON_AUTH_ENABLED=true npm run build` 成功、Playwright 全111件緑。

### 未検証区間

**本番で、実際にGoogleログインした状態から企画を保存するまでは未実測である。**
理由: この環境に実Googleアカウントの資格情報が無く、ログインできない。
確かめる手段: ユーザーが本番で1回、金銭欄を空のまま「下書き保存」を押す。
赤い枠で「金銭条件の説明 / 精算方法」と出れば効いている。
...
ユーザーが「公開されていないぞ」と報告。調べると、企画が非公開なのではなく、**会員登録そのものが完了していなかった**。`/member/profile`のプロフィール完了フォームで「プライバシーポリシーを確認し、同意します」にチェックし送信した際のスクリーンショットで、赤枠のエラー「登録を完了できませんでした。時間をおいて再度お試しください。」を確認した。

### 診断

- このエラー文言は`src/app/member/profile/actions.ts`の`completeProfileAction`のcatch節（59行目付近）から出る、DBトランザクション内で何らかの例外が起きたときの汎用メッセージである。
- **`確認済みメールアドレスが必要です`という別メッセージではなかった**ため、Google認証セッションの`emailVerified`は`true`で、以前から未確認扱いだったP15のrisk（Googleセッションが確認済み扱いになるか不明という懸念）はこれで解消・肯定された。
- catch節は例外を握り潰しており、**何が失敗したのかサーバーログにすら残らない**設計だった（P10で管理者操作には同種のログを追加していたが、この会員登録の経路は見落とされていた）。
- コードを静読した範囲（`profiles`/`consents`/`terms_versions`/`audit_log`のスキーマ、外部キー、型）では明確な不整合は見つからなかった。この関数自体は過去にメール+パスワード登録の会員2名で成功実績があるため、**Google認証切り替え(P15)後に初めて通った経路で、何かが新たに壊れている可能性が高い**が、断定はできない【曖昧】。

### 実施した対応

- `completeProfileAction`のcatch節に`console.error('completeProfileAction failed', ...)`を追加し、個人情報を含まない例外メッセージをVercelのログへ残るようにした。これにより次に失敗したときに原因を特定できる。
- この修正はログ出力のみで、登録ロジック自体は変えていない。

### ユーザーへのお願い（実機確認）

1. 本修正の本番反映後、`https://aiueo-lp.vercel.app/member/profile` を開き、もう一度「同意して会員登録を完了する」を押してください。
2. 成功すれば`/member`に「こんにちは、〇〇さん」の会員ページが出ます。そのまま企画登録に進めます。
3. 同じエラーが出たら、**Vercelの管理画面 → 対象プロジェクト → 上部タブ「Logs」** で、直近のログに`completeProfileAction failed`という行が無いか確認し、その行（の後半、エラー内容部分）をそのまま次のセッションへ共有してください。Vercelの画面にアクセスできない場合は、失敗した日時（何時何分ごろ）を教えてください。

### 検証

`npm run lint`/`npm run typecheck`は0件。ログ追加のみのためPlaywrightへの影響は無い想定（全111件を個別には再実行していない、次回コミット前に確認する）。

## 今回の作業（2026-09-09 その3 / P26 ログイン状態バッジ、Claude Code on the web）

ユーザーが実際にGoogleログインを試み、「許可」まで進めたあとトップページに戻ったが、**何も変わって見えず、ログインしているかどうか分からない**という報告を受けた。

### 診断（このセッションでは確認できた範囲）

- コード上、Googleログインの戻り先(`callbackURL`)は `/member/profile` であり、トップページではない（`src/components/register-form.tsx`）。トップページに戻ったとすれば、①ログイン後に自分で「AIueoへ戻る」リンク等を押した、②途中で失敗し`?auth_error=1`付きで`/register`へ戻った、のいずれかである可能性が高いが、**この環境からは実際に何が起きたか確認できない（曖昧）**。
- それとは別に、**サイト側に本物の不具合があった**。`navbar.tsx`はトップページ（`/`）にしか描画されず、しかもログイン状態を一切表示しない作りだった。`/register`や`/member/profile`など他のページには共通ナビ自体が無い。つまり**ログインに成功していたとしても、どのページを見ても「ログインした」というフィードバックが一切出ない**設計になっていた。これが今回の「わからない」の主因と判断し、今回はこちらを修正した。

### 実装

- `src/components/account-badge.tsx`を新設。サーバーコンポーネントとして`getAuthContext()`(既存のDAL、`src/lib/auth/dal.ts`)を呼び、全ページ共通の右上バッジを出す。
  - 未ログイン・認証基盤未設定のときは**何も描画しない**（既存の各ページの登録・ログイン導線を変えないため）。
  - ログイン済みだが会員登録未完了（`profile_missing`、またはstatusが`pending_profile`）: ゴールドの枠に`!`のバッジ。`/member/profile`（続きの入力画面）へリンク。
  - 有効会員（`status='active'`）: 公開名の頭文字を入れた塗りつぶしバッジ。`/member/profile`へリンク。
  - 停止・退会会員: 控えめな配色のバッジ。`/member/profile`（状態確認画面）へリンク。
- `src/app/layout.tsx`に`<AccountBadge />`を追加（`<SkipLink />`の直後）。位置は`fixed right-3 top-20`（トップページのナビ帯の高さ68pxを避けて、その下に固定）。
- ログアウト機能は今回のスコープに含めていない。**上流に`sign-out`エンドポイント自体は既に許可リストへ入っているが、押せるUIがサイト上のどこにも無い**ことをこの調査で確認した。次の課題として残す。

### 検証

`npm run lint`/`npm run typecheck`は0件。`NEXT_PUBLIC_NEON_AUTH_ENABLED=true npm run build`が成功し、ビルド出力で`/terms`・`/privacy`・`/disclaimer`・`/contact`・`/register`が引き続き静的（○）のままであること（バッジ追加で全ページが動的化していないこと）を確認した。Playwrightはこの環境の既定パスに実行ファイルが無かったため`CHROMIUM_PATH=/opt/pw-browsers/chromium-1194/chrome-linux/chrome`を指定して実行し、既存111件すべて緑（横スクロール・コンソールエラー・セキュリティヘッダ等の既存テストに回帰なし）。**開発用DB・実アカウントが無いため、実際にログインした状態でバッジが表示されることは未確認。**

### ユーザーへの回答（②企画登録の正解ルート）

1. ブラウザで `https://aiueo-lp.vercel.app/member/profile` を開く。
2. ログイン済みなら「会員登録を完了する」という見出しの画面が出る。公開名・協力したい内容・18歳以上の確認・3文書（会員規約・免責事項・プライバシーポリシー）への同意を入力して送信する。
3. 送信が終わると「登録内容」画面に切り替わる。そこにある「会員ページへ」ボタンを押すと `/member` に移動する。
4. `/member` にある「企画を登録する」ボタンを押すと `/member/proposals/new` で企画登録フォームに進める。

手順2で「確認済みメールアドレスが必要です」というエラーが出た場合は、GoogleログインのセッションがAIueo側の確認済み扱いになっていない**既知の未確認リスク**（`src/app/member/profile/actions.ts`の`emailVerified`判定）に当たった可能性が高い。その画面をそのまま次セッションへ共有してほしい。



P15（Google認証への切替）を完了した。**PR #46（`d2e62c8`）で`main`へマージ・本番反映済み。**

このセッションで、ユーザー側の外部設定（Google Cloud / Neon Console）とアプリ側の実装の両方が終わっている。**残るのはユーザーによる実機確認だけ**（下記「ユーザーへお願いする手動確認」）。
...
### 着手前に判明していた障害（すべて対応済み。経緯として残す）

Googleボタンを置くだけでは**ログインが成立しない**ことが、着手前の調査で分かっていた。

1. **`src/proxy.ts` が素通し**で、SDKのOAuth戻り処理 `exchangeOAuthToken` が一度も呼ばれない。
2. **許可リストに `sign-in/social` が無い**ため、Googleログインの最初のリクエストが404になる。
3. **`neonAuth.middleware()` をそのまま採用できない。** `skipRoutes` はSDK内の固定定数で差し替え不可。採用すると公開ページが全部ログイン必須になる。

当初は「OAuth戻りのときだけ middleware へ委譲する」設計にしたが、**敵対検証でこの設計自体を取り下げた**（下記「敵対検証と実操作テストで見つけて直したもの」3項）。交換に失敗した場合に公開ページが `/register` へ飛ばされる経路が残るためである。**現在は middleware を使わず**、認証SDKのクライアントが `getSession()` の中で行う交換を `src/components/oauth-session-sync.tsx` から呼ぶ形になっている。2は許可リストへ `sign-in/social` を追加して解消した。

### 敵対検証と実操作テストで見つけて直したもの

計画どおりに一度実装したあと、3視点の敵対検証と、実際にブラウザを動かす実操作テストをサブエージェントで回した。**当初の実装には、そのままでは Google ログインが成立しない欠陥が3件あった。**

1. **戻り先URLを相対パスで渡していた。** better-auth は値をそのまま `Location` に入れるため、ブラウザは**発行元（Neonのドメイン）を基準に解決**する。自分のサイトへ戻ってこない。→ `window.location.origin` を基準にした絶対URLへ変更。
2. **失敗時の戻り先を渡していなかった。** 同意のキャンセル、アカウント未連携などの失敗は、既定でNeon側の英語のエラー画面に着地する。戻る手段が無い。→ `errorCallbackURL` に `/register?auth_error=1` を渡し、`/register` 側でその文言を出すようにした。
3. **`src/proxy.ts` で middleware へ委譲する設計自体をやめた。** 「戻りのときだけ委譲する」条件を足しても、**交換に失敗した場合は保護判定まで進み、公開ページが `/register` へ飛ばされる**経路が残る。調べ直したところ、認証SDKのクライアントが `getSession()` の中で交換（キャッシュ回避・パラメータ転送・アドレスバーの掃除）を自前で行うことが分かったため、middleware を使わず `src/components/oauth-session-sync.tsx` を戻り先ページに置く形へ変更した。`proxy.ts` は素通しのままである。

あわせて次も直した。

- **同一オリジン検証のバグ（既存）。** `request.headers.get('origin') === request.nextUrl.origin` は、`127.0.0.1` と `localhost` の表記違いだけで正規の送信を403にする。**削除した `/api/membership/registration` も同じ比較で、成功する側のテストが無かったため見逃されていた。** Host ヘッダーとの突き合わせに変更し、成功・失敗の両方をテストで固定した。
- **書き込み経路の同一オリジン検証。** この Proxy は上流を呼ぶとき Origin を必ず付け直すため、上流側の検証が意味を失う。削除した登録APIが持っていた検証を、残った POST 経路へ移した。
- **失敗文言。** サーバー設定不備（503）でも「電波の状況を確認して」と出て、利用者のせいにしていた。原因を断定しない文へ変更。
- **規約・プライバシーポリシー。** Googleを名指ししていなかった（仕様書のプライバシーポリシー1項が要求）。Googleから渡る情報（メールアドレス・表示名・プロフィール画像・確認状態）と、Googleのポリシーが適用されることを明記。「氏名などの個人情報を収集・保管しません」という運営方針の文言も、Googleの表示名が本名であり得る以上そのままでは正しくないため修正した。
- **中身のないテスト。** 「待ち表示が出ていないこと」だけを見るテストは、実装が丸ごと無くても通る。交換リクエストが実際に飛ぶかを見る形へ書き換え、**コンポーネントを外すと落ちることを実測で確認した。**

### 実操作テストで確認できたこと（サブエージェントが実際にブラウザを動かした結果）

- ボタン押下で `POST /api/auth/sign-in/social` が飛び、本文に絶対URLの `callbackURL` と `errorCallbackURL` が入っている
- 失敗してもボタンは固まらず、理由が `role="alert"` で出る
- 戻り先ページに検証パラメータを付けると、実際に `get-session` へ交換リクエストが飛ぶ。失敗時はやり直し導線が出る
- 許可3経路は通過（認証未設定のため503）、メール認証4経路・管理API・メソッド違いはすべて404
- 外部オリジンからの POST は403、Origin なしも403
- メール・パスワードの入力欄は全ページで0件。ナビ・フッター・規約にも残骸なし
- `/register` は360/768/1280pxで横はみ出しなし、ボタン46px

**実際のGoogleアカウントを使った通しの確認は、この環境に認証情報が無いためできていない。** 下記の手動確認をユーザーが行う。
...
### ユーザーへお願いする手動確認（本番反映は完了済み。いつでも実施できる）

1. `https://aiueo-lp.vercel.app/register` を開く
2. 「Googleで続ける」を押す
3. Googleの画面に **`AIueo`** と表示されることを確認（自前の鍵が効いている証拠）
4. 自分のGoogleアカウントを選ぶ
5. AIueoへ戻り、**会員登録の入力画面（公開名・協力したい内容・3文書の同意）が出る**ことを確認
6. 入力して送信し、`/member` に登録内容が出ることを確認
7. 一度ログアウトし、もう一度入り直せることを確認

5でエラーが出たら、その画面をそのまま見せてもらう（`emailVerified` の追加対応へ進む）。

**この確認は 2026-09-09 時点で未実施である。** 本番反映（`d2e62c8`）は完了しており、`https://aiueo-lp.vercel.app/register` が「Googleで続ける」を返すことまでは実測で確認した。
...
P25（権限別の操作シナリオ100件）に対応。3本のPRを`main`へマージ済み。いずれもドキュメントのみで、コード・DB変更は無い。

- **PR #40（`8bccffe`）**: `docs/OPERATION_SCENARIOS.md` を新設し、シナリオ100件を列挙した。
- **PR #42（`74dafb5`）**: 同ファイルを、判定済みの一覧ではなく**実施できる手順書**へ整形した。各行に空の「結果」欄（`—`）を置き、冒頭に「これは手順書であり、検証結果ではない」「実施するのは人」を明記。実装を読んだ際の気づき7件は、判定ではなく**要確認**として末尾へ分離した。
- **PR #43（`62c2a55`）**: 台帳に残っていた整形前の記述（`HANDOFF.md`の「実コードのガード条件に紐づけて判定したもの」、`IMPLEMENTATION_PLAN.md`のP25行がPR #40しか挙げていなかった点）を訂正した。

`docs/OPERATION_SCENARIOS.md` を新設し、権限6区分（未ログイン／プロフィール未完了／有効会員／停止・退会会員／管理者／システム）について、オーソドックスな操作50件と致命的な危険操作50件を列挙した。

**これは手順書であり、検証結果ではない。実施するのは人で、エージェントには検証させない。** 各行の「結果」欄は空（`—`）のまま置いてある。実施した人が `OK` / `NG` / `不可` を書き込む。

### 実施に必要で、いま揃っていないもの

- **運営（管理者）アカウント**: 未作成。管理者のシナリオ（A-39〜A-47、B-37〜B-45）は実施できない
- **有効会員のアカウント2つ以上**（1つは停止措置の対象、もう1つは他人の企画を用意するため）: 未作成
- **開発用またはテスト用のデータベース**: 無し。本番DBで実施するか、別途用意する

### 参考: シナリオを書くために実装を読んだ際の気づき（検証結果ではない）

期待どおりに止める処理が**見当たらなかった**箇所。実際にそうなるかは、該当シナリオを実施して確かめる必要がある。詳細は `docs/OPERATION_SCENARIOS.md` 末尾の表。

B-37（管理者の自己停止を止める処理）、B-29（開催状況ボタンの確認処理）、B-46（DB断時の画面表示）、B-30（通報の回数制限）、B-49（ログインの回数制限）、B-26〜B-28（新規作成側の日時検証）、B-36（退会者の再登録の判別）。

### 既知として台帳に記載済みの未達（シナリオ表にも収録）
...
P24（アクセシビリティ Tier 4 の現存11件を修正）に対応。PR #38（`f899bf2`）で`main`へマージ・本番反映済み。
...
P19の敵対検証（2026-09-06）で見つかり、対応を持ち越していた仕様書側の矛盾。企画登録フロー4項が状態一覧に`submitted`（公開確定処理中）を含んでいたが、`submitted`は2026-09-05のP10で「DBのCHECK制約に存在しないのに管理画面の選択肢に出ていた」旧Supabaseスキーマの名残として、`src/app/admin/statuses.ts`の選択肢から既に除去済みだった。同項は実在する状態`auto_hidden`も欠いていた。さらに企画登録フロー7項は「開催候補日の3日前になっても`confirmed`でない場合...公開状態を`hidden`にする」と書いていたが、実装（`process_proposal_deadlines`）と必須設計4項は`auto_hidden`（企画者が再編集・再公開できる）を使っており、管理者の緊急非公開`hidden`（企画者は編集不可）とは別の状態として扱っている。仕様書の文言が実装に追いついていなかった。

### 実装

- `MEMBERSHIP_FEATURE_SPEC.md`企画登録フロー4項: 状態一覧から`submitted`を削除し`auto_hidden`を追加。「`submitted`は管理者の審査待ちではない」の一文は、`submitted`という語自体が実装に存在しないため「公開は管理者の審査待ちを経ない」と表現を改めた（意味は変えていない）。
- 同7項: 3日前の自動除外先を`hidden`から`auto_hidden`に訂正し、「管理者による緊急非公開（`hidden`）とは別の状態として扱う」を追記して今後の混同を防いだ。
- 文書冒頭の「最終更新」注記を今回の修正内容に更新。

### 検証

`grep`で`src/`配下に`submitted`の実装参照が`src/app/admin/statuses.ts`のコメント（除外理由の説明）1件のみであることを確認し、コード側の変更は不要と判断した。ドキュメントのみの変更のため、CI（lint/typecheck/build/Playwright）が通ることのみ確認する。
...
- `src/components/profile-completion-form.tsx`の「公開名」欄の説明文を、実態（公開名は会員ページにのみ表示され、企画ページの主催者名は`organizer_name`という別項目）に合わせて修正。
- `src/app/events/[slug]/page.tsx`に`MoneyConditions`コンポーネントを追加し、`JSON.stringify(data.money_details ?? {})`の生出力を、金銭の種類（日本語ラベル）＋各項目（説明・金額・支払先等）を見出し付きで並べる表示に変更。存在するキーだけを表示し、欠損キーは出さない。
- `src/app/member/history/page.tsx`を全面改修。固定文言9行だけだったページに、自分の企画一覧（掲載状態・開催状況・最終更新日時）、企画ごとの管理者とのメッセージ、規約同意履歴（`consents`×`terms_versions`）をDBから取得して表示するようにした。会員が停止・退会中（`status !== 'active'`）の場合は、個別の企画ページ（`/member/proposals/[id]`、`requireActiveMember()`でガードされ停止会員はアクセスできない既存の別制約）へのリンクを出さず、プレーンテキストで表示することで、押しても弾かれるリンクを作らないようにした。

### 検証

`npm run typecheck`/`lint`/`NEXT_PUBLIC_NEON_AUTH_ENABLED=true npm run build`/Playwright全95件、いずれも成功。**開発用DBが無いため、実データでの表示確認（企画・メッセージ・同意履歴が正しく出ること）は未実施。**

### 未解決事項として持ち越し

- `/member/history`から個別の企画・メッセージへ深掘りする経路（`/member/proposals/[id]`等）自体は、`requireActiveMember()`により停止・退会会員がアクセスできない。今回は`/member/history`側でリンクを出し分けることで回避したが、根本的には該当ページ群のガードを「読み取りはstatus不問、書き込みのみactive限定」に見直す余地がある。仕様書「必須設計」5項をより厳密に満たすなら次の課題。

## 今回の作業（2026-09-06 その5 / チェックイン、Claude Code on the web）

設計図の不足#3（企画の編集・下書きからの公開手段が無い）に対応。PR #28（`5198d8b`）で`main`へマージ・本番反映済み。
...
`npm run typecheck`/`lint`/`build`/Playwright全95件、いずれも成功。

## 今回の作業（2026-09-06 その3 / チェックイン、Claude Code on the web）

設計図の不足#2（トップページの「進行中の企画」がDB未接続）に対応。PR #24（`d34f539`）で`main`へマージ・本番反映済み。

### 決定事項（ユーザー承認済み）

- トップページの「進行中の企画」を`mock.ts`の見本から、`/events`と同じDBの公開企画データへ差し替える。企画公開後は**即座に**トップページへ反映される（`force-dynamic`、`/events`と同じキャッシュ設定）。
- 公開企画が0件のときはセクション自体を非表示にする（「準備中」等の文言も出さない）。
- タグ絞り込みボタン（教育/コミュニティ/開発/創作）は削除する。DB側に分類情報が無く、`tags:[]`のままだと「ALL」以外のボタンが恒久的に無反応になるため。
- 企画カードの画像は、DBに画像列を追加せず、`mock.ts`にある既存の写真素材（`seminar.png`/`workshop.png`/`development.png`/`meetup.png`）を企画の並び順にそのまま割り当てる（企画ごとの画像アップロードは未実装のまま）。
- 「満席」「金銭条件あり」等のラベルは出さない（タイトル・概要・日程・詳細リンクのみのシンプル表示）。

### 実装

- `src/lib/proposals/public.ts`を新設し、`/events`にあった公開企画取得クエリ(`PublicProposal`型・`getPublicProposals()`)を共有DAL関数として切り出した。SQL条件は変更していない。
- `src/app/events/page.tsx`はこの共有関数を呼ぶだけに変更（振る舞い不変のリファクタ）。
- `src/app/page.tsx`を`async`化し、`force-dynamic`を追加。DBの公開企画を`Activity`型へ変換するローカル関数`toActivity`は、フィールドを明示的に列挙し(`money_details`/`organizer_name`等の非公開ページ用フィールドを混入させない設計)、画像は上記4枚を順番に割り当て、`actionLabel`は「詳細を見る →」（旧「参加・合流する」から、当事者性を示唆しない表現へ変更）、`actionUrl`は`/events/${slug}`。
- `src/components/upcoming-events.tsx`からタグ絞り込みボタンとフィルタリングロジックを削除。
- 公開企画0件時にセクション(`id="events"`)が消えると、`src/components/who-we-are.tsx`と`src/components/footer.tsx`の`#events`アンカーリンクが押しても反応しなくなる不具合を敵対検証で発見。両方とも`/events`への通常リンクに変更して修正した。

### 敵対検証（3視点、サブエージェントで実施）

- **セキュリティ/権限**: 重大な問題なし。公開条件(`status='published' and visibility='public' and public_expires_at > now()`)はスキーマ上安全。変換は明示的フィールド列挙にする（受け入れ条件として反映済み）。
- **企画者・参加者の利用**: タグ絞り込みボタンの恒久的無反応、`#events`アンカーリンクの破損（本番投入後すぐ発生する具体的なリンク切れ）を発見。いずれも対応済み。
- **運用・法務**: 「満席」(`event_status='full'`)・金銭条件バッジを表示しないことは仕様違反ではないが、既存`/events`一覧より情報量が減る点、CTA文言「参加・合流する」がAIueoの当事者性を示唆する点を指摘。文言は「詳細を見る →」に変更。満席・金銭バッジは今回は出さない方針をユーザーが確定。

### 検証

`npm run typecheck` / `npm run lint`（要修正1件: `who-we-are.tsx`の`<a href="/events">`を`next/link`の`<Link>`に変更）/ `NEXT_PUBLIC_NEON_AUTH_ENABLED=true npm run build` / Playwright全95件、いずれも成功。ローカルはDB未接続のため、企画0件時の挙動（セクション非表示、`/events`リンクが正しく機能、コンソールエラー無し）のみ実測確認。**公開企画がDBに実在する状態での見た目確認は未実施**（運営アカウント・実企画が現時点で1件も無いため）。

### 未解決事項として持ち越し

- 設計図の不足#3（企画の編集・下書きからの公開手段が無い）、#4（ナビ/フッターの`#join`が`/register`に繋がっていない）は今回は対象外。
- Google認証切替（P15、不足#1）は外部設定（Google Cloud Console + Neon Console）の案内を実施したが、ユーザー側の設定作業は未完了。
...
- **G1-05を決定**: 3日前未確定の企画は`auto_hidden`（論理削除・公開から除外・履歴保全）とし、完全消去はしない。`MEMBERSHIP_FEATURE_SPEC.md`企画登録フロー7項・`process_proposal_deadlines`の実装は決定前から既にこの内容だったため、**コード変更は無い**。
- **G1-06を決定**: 管理者は全企画の全項目（金銭条件・日程・主催者情報を含む）を編集できるが、版履歴・変更差分・理由・企画者への即時通知を必須とする。`MEMBERSHIP_FEATURE_SPEC.md`敵対検証を受けた公開前の必須設計3項・管理者編集の実装は決定前から既にこの内容だったため、**コード変更は無い**。
...
コード側で片付けられる項目は設計図の不足11件・監査Tier 1〜4ともすべて解消済みで、**Google認証（P15）のアプリ側実装も完了し、`main`へマージ・本番反映済みである**。残るのは**ユーザー側の外部作業か、人による実施**である。

| 残件 | 何が要るか | 誰がやるか |
| --- | --- | --- |
| シナリオ100件の実施（P25） | 運営アカウント、有効会員アカウント2つ以上、テスト用DB | 人（**エージェントには検証させない**方針） |
| **Google認証の実機確認（P15）** | 本番反映は完了済み。実際のGoogleアカウントで1回ログインする | ユーザー（手順は本ファイル「ユーザーへお願いする手動確認」） |
| 運営アカウントの作成（不足#5） | 初期管理者の付与作業。`docs/ADMIN_BOOTSTRAP_RUNBOOK.md`は旧Supabase前提で要更新 | ユーザー |
| 通知メールの送信（P6、不足#6） | G1-02（送信元アドレス・送信サービス）の決定が先 | ユーザーの決定 → 実装 |

上記が揃えば、P7（横断受入・本番）へ進める。シナリオの実施結果は `docs/OPERATION_SCENARIOS.md` の「結果」欄へ直接書き込む。

### 最優先: 設計図の不足11件を、番号順に片付ける（#2・#3・#4・#7〜#11は解消。残り2件）

**→ https://claude.ai/code/artifact/0de7067b-8736-4325-bf09-ebe7dab72830** （リンク先の記載は公開時点のまま。解消済み分は本ファイル上部の「今回の作業」「確定した問題リスト」を参照）

「企画を立てる道」「参加する道」は最後までつながった。**1（Google認証）は2026-09-09に解消した**（PR #46、`d2e62c8`。外部設定・アプリ実装とも完了し本番反映済み。残るのはユーザーによる実機確認のみ）。残るのは5（運営アカウント、外部作業待ち）で、ユーザー側の外部作業が必要。6（通知メール未送信）はG1-02未決定のため送信基盤そのものが無い。**#3の実装で見つかった関連バグは、管理者措置理由の非表示（P22）・仕様書の用語矛盾（P23）とも解消済み。残るのは#8〜#10の実装で見つかった`/member/proposals/[id]`系ページの停止会員アクセス不可のみ（「今回の作業」の「未解決事項」を参照）。**
...
2026-09-06に対応済み（PR #28、コミット`5198d8b`、`main`へマージ・本番反映済み）。`/member/proposals/[id]`から企画内容を編集し、下書き⇔公開を切り替えられるようにした。新規作成時に`money_type='undecided'`のまま公開できてしまう検証漏れ(#11)も同時に修正。**本番URLで実測確認済み**: `/member/proposals/new`が未認証で307リダイレクト(既存の認可パターンどおり)。**運営アカウント・実企画が現時点で1件も無いため、所有者チェック・状態遷移・日時検証の実DB上での動作確認はまだ。**

### 解消済み: トップページの「進行中の企画」をデータベースにつなぐ（不足 #2）

2026-09-06に対応済み（PR #24、コミット`d34f539`、`main`へマージ・本番反映済み）。`/events`と同じDBデータを表示し、0件時はセクション非表示、タグ絞り込みボタンは削除、画像は`mock.ts`の既存素材を流用。**本番URLで実測確認済み**: `id="events"`セクションが無く(企画0件のため非表示)、`href="/events"`リンクは正常、タグ絞り込みボタンの文言も残っていない。実企画データが入った状態での見た目確認はまだ（運営アカウント・実企画が現時点で1件も無いため）。
