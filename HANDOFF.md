...
- `src/lib/proposals/headcount.ts` — 入力の判定と、画面に出す1行の作り方を1か所に集めた。
  ...
  - 定員なし … 「参加 3人」。0人なら**何も出さない**（空の行を作らない）
...
| 見たこと | 実測 |
| --- | --- |
| 参加人数の経路（実ブラウザ12項目） | **すべてOK** |
| うち「企画の中に入る前に出る」 | 一覧カード・トップページとも「参加 3 / 10人（残り7）」 |
| うち「満席」 | 定員まで埋めると「参加 10 / 10人（満席）」 |
...
`https://aiueo-lp.vercel.app/events` のカードに「参加 3 / 10人（残り7）」と出ることを見る。
