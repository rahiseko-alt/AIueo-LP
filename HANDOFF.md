...
- **P34（2026-09-12）: 通報を1階層下げ、画像添付を追加し、立場を3つに統一し、導線12件を直した。PR #59（`22c8951`）で`main`へマージ・本番反映済み。`drizzle/0003`も本番適用済み。** 実操作の検証で、修正前は25項目中12項目が到達不能だった（例: 公開中の自分の企画の公開ページを見る手段が無い、企画編集から来た道へ戻れない、企画詳細からトップへ戻れない）。修正後は40項目すべてOK。検証は `bash scripts/local-verify-rig.sh` で1コマンドで再現できる。**ログインの要らない範囲は本番でも実測した**（下記）。**未検証区間: ログインが必要な操作 — 画像を選んで保存する、会員側の画面（`/member` 配下）の導線（理由: この環境に実Googleアカウントの資格情報が無い。確かめる手段: ユーザーが本番でログインし、企画編集画面で画像を選んで「下書き保存」を押し、公開ページにその画像が出ることを見る）**
...
**ログインが必要な操作は、本番では未実測である。**
具体的には、(1) 企画編集画面で画像を選んで保存し公開ページに出ること、
(2) 会員側の画面（`/member` 配下）の導線。
理由: この環境に実Googleアカウントの資格情報が無い。
確かめる手段: ユーザーが本番でログインし、自分の企画の編集画面で画像を選んで「下書き保存」を押し、
公開ページにその画像が出ることを見る。
