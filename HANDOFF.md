...
コード側で片付けられる項目は設計図の不足11件・監査Tier 1〜4ともすべて解消済みで、残るのは**ユーザー側の外部作業か、人による実施**である。エージェント側から着手できるものは無い。

| 残件 | 何が要るか | 誰がやるか |
| --- | --- | --- |
| シナリオ100件の実施（P25） | 運営アカウント、有効会員アカウント2つ以上、テスト用DB | 人（**エージェントには検証させない**方針） |
| Google認証への切替（P15、不足#1） | Google Cloud Console と Neon Console での設定 | ユーザー |
