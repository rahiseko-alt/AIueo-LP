...
- **P34（2026-09-12）: 通報を1階層下げ、画像添付を追加し、立場を3つに統一し、導線12件を直した。** 実操作の検証で、修正前は25項目中12項目が到達不能だった（例: 公開中の自分の企画の公開ページを見る手段が無い、企画編集から来た道へ戻れない、企画詳細からトップへ戻れない）。修正後は40項目すべてOK。検証は `bash scripts/local-verify-rig.sh` で1コマンドで再現できる。**未検証区間: 本番で実ログイン状態での操作（理由: この環境に実Googleアカウントの資格情報が無い。確かめる手段: ユーザーが本番で企画を開き、「管理者へ連絡」が下部のボタンになっていること・画像を付けて保存できること・企画一覧⇄詳細⇄公開ページを行き来できることを見る）**
...
**本番で、実際にGoogleログインした状態での操作は未実測である。**
理由: この環境に実Googleアカウントの資格情報が無い。
確かめる手段: ユーザーが本番で企画を1つ開き、(1)「管理者へ連絡」が下部のボタンになっていること、
(2) 企画編集画面で画像を選んで保存し、公開ページに出ること、(3) 企画一覧 ⇄ 企画詳細 ⇄ 公開ページを
行き来できることを見る。
...
- **`drizzle/0003_proposal_images.sql` は本番Neonへ未適用である。** マージ前に本番DBへ当てること。当てないと、企画の保存・編集・状態変更がすべて「列が無い」で失敗する（`image_data` / `image_mime` / `image_updated_at` を参照するため）。順序を逆にしてはならない。
