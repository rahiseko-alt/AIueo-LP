...
- 最新の実装コミット: `855372a fix: 敵対検証と実操作テストの指摘を反映する(P15)`（**ブランチ `claude/checkin-6hrtds` にあり、`main` へは未マージ**。マージまでは本番へ反映されない）
- `main` へ入っている最後の実装コミット: `f899bf2 fix: アクセシビリティTier 4の現存11件を修正する(P24) (#38)`
- 最新の`main`コミット: `62c2a55 docs: 台帳のシナリオ記述をPR #42の内容へそろえる(P25) (#43)`（2026-09-09。P25以降はドキュメントのみで、アプリのコードは`f899bf2`から変わっていない）
...
## 今回の作業（2026-09-09 その2 / P15 Google認証の設定案内、Claude Code on the web）

P15（Google認証への切替）の外部設定を、ユーザーがGoogle Cloud側で開始した。エージェント側はコードを触っておらず、案内のみ。

### 進捗（ユーザー側の外部作業）

- Google Cloud プロジェクト `aiueo-lp` で **Google Auth Platform のブランディング作成が完了**（アプリ名 `AIueo`、サポートメールはアカウントのアドレス）。作成時にエラー表示が出たが、その後の`OAuth の概要`画面と左メニュー（ブランディング／対象／クライアント）の出現から、実際には作成できていると判断した。
...
コード側で片付けられる項目は設計図の不足11件・監査Tier 1〜4ともすべて解消済みで、**Google認証（P15）のアプリ側実装も完了した**（マージ待ち）。残るのは**ユーザー側の外部作業か、人による実施**である。

| 残件 | 何が要るか | 誰がやるか |
| --- | --- | --- |
| シナリオ100件の実施（P25） | 運営アカウント、有効会員アカウント2つ以上、テスト用DB | 人（**エージェントには検証させない**方針） |
| **Google認証の実機確認（P15）** | 本番反映後、実際のGoogleアカウントで1回ログインする | ユーザー（手順は本ファイル「本番反映後にユーザーへお願いする手動確認」） |
...
「企画を立てる道」「参加する道」は最後までつながった。残るのは1（Google認証、外部設定待ち）と5（運営アカウント、外部作業待ち）で、いずれもユーザー側の外部作業が必要。6（通知メール未送信）はG1-02未決定のため送信基盤そのものが無い。**#3の実装で見つかった関連バグは、管理者措置理由の非表示（P22）・仕様書の用語矛盾（P23）とも解消済み。残るのは#8〜#10の実装で見つかった`/member/proposals/[id]`系ページの停止会員アクセス不可のみ（「今回の作業」の「未解決事項」を参照）。**
