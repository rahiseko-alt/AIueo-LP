...
- **選択肢のある質問は、必ず押せるボタン（`AskUserQuestion`）で出す。** 本文に
  「A／B／C のどれにしますか」と箇条書きで並べるのは禁止。打ち返す手間がかかる。
  3問あるなら1回の呼び出しに3問まとめる。詳しくは `AGENTS.md` §8。
  （2026-09-18、ユーザーの指示。「返答が手間。ボタンを押す形式にしてほしい」）
...
- **2026-09-17: メンバー紹介の左から2番目の写真を差し替えた。** ユーザー提供の写真。
  ...
  **実際に踏んだ**（名前を変える前は、ファイルを置き換えても画面は古い写真のままだった）。
  **画像を差し替えるときは、必ずファイル名も変えること。**
  名前と肩書もユーザーの指定で「**伊藤良二 / AI顧問**」へ直した（見本の
  「Yuka Takahashi / Life & Family AI Lecturer」から）。**このカードだけ日本語表記になっている。**
  他の3枚は見本のままローマ字（`Kouhei Kosehira` ほか）で、そろえるかはユーザーの指示待ち。
  **カードの中身も差し替えた。** それまでは「Collaborated on:」として見本の活動名が
  並んでおり、**本人がやっていない企画を実績のように見せていた**。`Person` に任意の
  `bio` を足し、入っている人はその紹介文を出して一覧を出さないようにした。
  伊藤良二のカードには本人の紹介文（「子どもにも仕事にもAI活用をやさしく伴走します。…」）を
  入れ、`activityIds` は空にした。

- **2026-09-17（続き）: トップから架空の人物と架空の実績を全部外した。**
  ユーザーの指示「kouheikosehira.com見て良い感じにしとけ」を受け、**本人のサイトを実ブラウザで
  開いて中身を読み**、そこにある事実だけに差し替えた。
  - **メンバーは4枚→2枚。** 架空の「Taro Yamada」「Kenichi Suzuki」を削除した。残るのは
    「小齊平 恒平 / 主宰・業務改善・自動化・AI活用支援」と「伊藤良二 / AI顧問」で、
    **どちらも日本語表記になり、ローマ字との混在は解消した**（保留にしていた表記の件はこれで決着）。
  - **03 セクション（`LOGS & WHAT WE DID` / `Recent Activities`）の中身を、主宰の実績へ差し替えた。**
    それまでは「世田谷で24名」「鎌倉で18名」など、**日付も人数も場所も架空の記録**が3件並んでいた。
    いまは kouheikosehira.com の「主な開発事例」から4件（SNS運用の自動化／AIチャットボット／
    音声カルテ作成アプリ／書類業務の半自動化アプリ）。参加人数と共同メンバーの欄は**出さない**
    （捏造しやすいので持たせない）。日付の代わりに効果（「月50時間相当を削減」など）を出す。
  - `mockUpcomingActivities`（架空の開催予定4件）も削除した。画面には出ていなかったが、
    残しておくと次に誰かが使ってしまう。
  - **見出し・ナビの文言は一切変えていない。** 途中で「AIueoの企画ではありません」という
    断り書きと英語見出しの変更を画面へ入れたが、**ユーザーから「変なこと書くなよ」と指摘を受けて
    すべて取り消した**。断り書きをページに載せるのは読み手にとって不自然である。
    同じ注意は `recent-log.tsx` のコード内コメントに残した（画面には出ない）。
  - **写真は用意のある素材を当てているだけで、案件そのものの写真ではない**（コメントにも書いた）。
...
- **P37（参加人数）は、安全な順番で入れ直して本番反映済み（2026-09-15）。** 順番はこう守った。
  ①窓口へ2列を `pending` で登録して先に出す → ②ユーザーが `drizzle/0005` を本番Neonへ適用 →
  ③窓口で `pendingMissing` が空になったことを**こちらで機械的に確認**（`proposals.capacity` と
  `proposals.participant_count` がともに `present: true`）→ ④コードをマージ → ⑤窓口の2行を `required` へ移動。
  **1度目の事故で欠けていたのは③である。**


- 本番URL: https://aiueo-lp.vercel.app/
- Vercelプロジェクト: `rahisekos-projects/aiueo-lp`
- 最新の実装コミット（1つ前）: `48e7edc feat: 参加申し込みフォームのURLを足し、監査記録への画像混入を直す(P35/P36) (#62)`（`main`へマージ済み・**本番反映済み**。`drizzle/0004`も本番適用済みで、適用をこちらで機械的に確かめた）
...
- **P34（2026-09-12）: 通報を1階層下げ、画像添付を追加し、立場を3つに統一し、導線12件を直した。PR #59（`22c8951`）で`main`へマージ・本番反映済み。`drizzle/0003`も本番適用済み。** 実操作の検証で、修正前は25項目中12項目が到達不能だった（例: 公開中の自分の企画の公開ページを見る手段が無い、企画編集から来た道へ戻れない、企画詳細からトップへ戻れない）。修正後は40項目すべてOK。検証は `bash scripts/local-verify-rig.sh` で1コマンドで再現できる。**本番でも実測した**（下記）。**画像は端から端まで本番で通った**（2026-09-12、ユーザーが実際に写真を添付し、こちらで配信された実物を取得して確認: HTTP 200 / `image/jpeg` / 89,357バイト / **1280×853px**＝設計どおり長辺1280へ縮小されている。企画詳細・企画一覧・トップページのいずれにも出ている）。**残る未検証区間: 会員側の画面（`/member` 配下）の導線が本番でも期待どおりかは未実測（理由: この環境に実Googleアカウントの資格情報が無い。ローカルの検証装置では40項目すべてOK）。ただし画像の保存が通ったことから、ログイン・企画編集画面・保存の経路自体は本番で動いていることが確かめられている。**
...
- 品質ゲート: Playwright **114件**（P15で13件、P34で3件追加）
...
- 品質ゲート: `lint` / `typecheck` / `build` / Playwright 114件が GitHub Actions で PR ごとに必須実行され、緑
...
## 今回の作業（2026-09-14 / P37 企画の一覧に参加人数を出す、Claude Code on the web）

ユーザーの指示は「参加者を企画の表に表示する機能つけろ。企画の中に入る前」。

### 先に決めてもらったこと

企画の表に出せる「参加者」の情報が、DBに1つも無かった（`proposals` に人数・定員・参加者の
列なし）。出し方で作るものが変わるため、3案を見せて選んでもらった。

**選ばれたのは「人数だけ（企画者が入力）」。** 応募者の氏名・連絡先はAIueoに保存しない
（`MEMBERSHIP_FEATURE_SPEC.md` 実装前提のまま。仕様は変えていない）。
数を入れるのは企画者本人で、AIueoが自動で数えることはしない。

### 追加したもの

- `drizzle/0005_proposal_headcount.sql` — `capacity`（定員、未設定＝上限なし）と
  `participant_count`（いまの参加人数、既定0）。CHECK制約で範囲を守る。
  **1つの `do` ブロックなので、Vercelの画面へ1回貼るだけで済む。**
- **言葉は「参加人数」で統一し、主催者を含めた数とした**（ユーザーの指示）。
  「参加者」と書くと、主催者だけなのか別に参加者がいるのか読み手に分からない。
  **1なら主催者だけ、2なら主催者ともう1人。** 定員も主催者を含めた上限として扱う。
  企画者の入力欄と、参加を考える人が見る企画詳細の両方にその旨を書いた。
- `src/lib/proposals/headcount.ts` — 入力の判定と、画面に出す1行の作り方を1か所に集めた。
  - 定員あり … 「参加人数 3 / 10人（残り7）」。埋まっていれば「満席」
  - 定員なし … 「参加人数 3人」。0人なら**何も出さない**（空の行を作らない）
  - 見出しで既に「参加人数」と書いてある企画詳細では、数の部分だけを出す（`headcountText`）
  - **定員を超える人数も拒まない。** 主催者が実際に定員より多く受け入れることがあるため。
    その場合は「満席」と出る
- 出す場所は3つ。**企画の一覧（`/events`）**、**トップページのカード**、企画詳細。
  トップのカードは既存の `spots` 欄へ流し込んだので、部品側の変更は要らなかった。
- 企画者の編集画面と、**管理者の編集画面の両方**に欄を置いた（G1-06）。
- 企画詳細には「主催者が入力した数です。最新かどうかは主催者へご確認ください。」と添えた。

### 途中で見つけて直したもの

**新しい2欄の日本語名を登録し忘れていた。** 定員に0を入れるとブラウザが送信を止めるが、
画面には英語の `capacity` と出てしまう。`FIELD_LABELS` へ「定員」「いまの参加人数」を追加した。
あわせて、この通知の文言が「次の項目が**空**のため」だけだったのを
「空、または入力が正しくないため」へ直した（範囲外の値のときに実態と食い違うため）。

### 検証（肯定・否定・おかしな値の3方向を実測）

| 見たこと | 実測 |
| --- | --- |
| 参加人数の経路（実ブラウザ13項目） | **すべてOK** |
| うち「企画の中に入る前に出る」 | 一覧カード・トップページとも「参加人数 3 / 10人（残り7）」 |
| うち「主催者を含む」の明記 | 企画者の入力欄・企画詳細の両方に出ている |
| うち「満席」 | 定員まで埋めると「参加人数 10 / 10人（満席）」 |
| うち「空なら行が出ない」 | 定員も人数も空なら人数の行自体が出ない |
| うち「定員0」 | 保存されず、画面に**日本語で**「定員」と出る |
| DB側のCHECK | 定員0・人数マイナス・上限超え をすべて拒否 |
| サーバー側の判定（実モジュールへ直接） | 全角「１０」・小数・マイナス・文字・0・上限超え をすべて拒否、正しい値は通る |
| **否定側**: 一覧から人数の表示を外す | H-04・H-07・H-08 がNGになり、全体もNG |
| 操作経路40項目・画像10項目・申し込みURL10項目 | 変わらずすべてOK（回帰なし） |

品質ゲート: `lint` 0件、`typecheck` 0件、`NEXT_PUBLIC_NEON_AUTH_ENABLED=true build` 成功
（`/terms` 等の静的ページが動的化していないことも出力で確認）、Playwright 全114件緑。

再現手段: `bash scripts/local-verify-rig.sh`（`scripts/verify-headcount.mjs` を追加した）。

### 検証で自分が間違えた点（申し送り）

最初、全角の数字を画面から入れて弾かれることを確かめようとしたが、
**入力欄の種類（数字専用）がブラウザ側で全角を受け付けない**ため、そもそも打てなかった。
欄の種類を書き換えて送る細工も、画面が組み直される際に元へ戻って効かなかった（実測）。
**画面から実際に送れるのは「半角だが範囲外」の値だけ**である。そこで、画面からはそちらで
確かめ、届いてしまった場合の判定は実モジュールへ直接渡して確かめる形に分けた。

### 未検証区間

**本番で、実際にログインした状態から人数を入れるところは未実測である。**
理由: この環境に実Googleアカウントの資格情報が無い。
確かめる手段: ユーザーが本番で自分の企画を開き、「定員」と「いまの参加人数」を入れて保存し、
`https://aiueo-lp.vercel.app/events` のカードに「参加人数 3 / 10人（残り7）」と出ることを見る。

**`drizzle/0005_proposal_headcount.sql` は 2026-09-15 に本番Neonへ適用済みである。**
適用は `/api/health/schema` でこちらが機械的に確かめた。
（この節を書いた時点では未適用で、その状態でマージして本番を落とした。経緯は F-11。）

## 今回の作業（2026-09-13 / P35 参加申し込みフォームURL、P36 監査記録への画像混入の修正、Claude Code on the web）

ユーザーの質問は「応募が来た時に、応募者を管理する機能はあるか？」。答えは**無い**。
AIueoは申し込みを受け取らない仕様（`MEMBERSHIP_FEATURE_SPEC.md` 実装前提「参加申込も初期版では
外部フォームまたは主催者指定の方法へ委ねる」）で、`proposals` にも応募者を入れる列は無い。
そのうえでユーザーが選んだのが「A」＝**仕様は変えず、主催者が自分で用意したフォームへ
行きやすくする**である。

### P36 先に見つけた不具合（P34で自分が入れた対策の漏れ）

Aの実装で管理者の編集フォームを読んだとき、`recordAdminChange` に企画の行が**そのまま**
渡っているのを見つけた。推測で断定せず、本番と同じ4マイグレーションを当てたPostgreSQL 16で
実測した。

| 見たこと | 実測 |
| --- | --- |
| 89,357バイトの画像を付けた企画に、管理者が状態変更を1回 | `moderation_actions` の before 358,293／after 358,268 バイト |
| 同じ操作の `audit_log` | before 358,293／after 358,268 バイト |
| **1操作あたりの合計** | **1,433,122 バイト**（元画像の16倍） |
| 入っていた中身 | `image_data` が `{"data":[65,65,…],"type":"Buffer"}` |

P34で入れた `withoutImageData` は `proposal_versions` にしか掛かっていなかった。
`src/app/admin/actions.ts` の94行目・182行目にも通した。

| 修正後に見たこと | 実測 |
| --- | --- |
| 同じ操作の `moderation_actions` | **815 / 818 バイト**（440分の1） |
| `image_data` キー | 無し。代わりに `has_image: true` |
| 企画に付いている画像 | 89,357 バイトのまま（消えていない） |

`profiles` と `reports` を渡す2か所（115行目・130行目）は画像の列を持たないので触っていない。

### P35 追加したもの

**企画に「参加申し込みフォームのURL（任意）」を1つ持たせた。** Googleフォームなど、主催者が
自分で用意した申し込みページのURLを貼ると、公開ページに「参加を申し込む（外部サイトが
開きます）」ボタンが出る。**応募者の氏名・連絡先はAIueoに一切保存しない。** 画面にもその旨を
書いてある（「申し込んだ人の名前や連絡先はAIueoには届きません」）。

- `drizzle/0004_proposal_application_url.sql` — `application_url` 列とCHECK制約。
  **全体を1つの `do` ブロックにしてあるので、Vercelの画面へ1回貼るだけで済む。**
  何度流しても結果は変わらない。
- `src/lib/proposals/application-url.ts` — `http(s)://` 以外、2000字超、空白入りを拒み、
  理由を日本語で返す。`javascript:` や `data:` を公開ページのリンクに置くと、押した人の
  画面で任意のコードが動きうるため。**DB側にも同じ条件のCHECK制約がある**（二重化）。
- 企画者の編集画面と、**管理者の編集画面の両方**に欄を置いた（G1-06「管理者は全項目を
  編集できる」）。
- 参加方法欄のプレースホルダから「外部フォームURL」を外した（専用の欄ができて重複するため）。

### 検証（肯定・否定・危険URLの3方向を実測）

| 見たこと | 実測 |
| --- | --- |
| DB側のCHECK制約 | `javascript:alert(1)` 拒否／`https://example.com/form` 受理／2000字超 拒否 |
| 申し込みURLの経路（実ブラウザ10項目） | **すべてOK** |
| うち「押すと別タブでそのURLが開く」 | 実際に新しいタブが開き、URLが一致 |
| うち「`javascript:` は保存されない」 | 画面に理由が出て、DBの値も変わらない |
| うち「空欄に戻すとボタンが消える」 | DB `null`／画面のボタン0個 |
| **否定側**: 公開ページからボタンの区画を外す | A-04 がNGになり、全体もNGになる |
| 操作経路40項目・画像10項目 | 変わらずすべてOK（回帰なし） |

品質ゲート: `lint` 0件、`typecheck` 0件、`NEXT_PUBLIC_NEON_AUTH_ENABLED=true build` 成功
（`/terms` 等の静的ページが動的化していないことも出力で確認）、Playwright 全114件緑。

再現手段: `bash scripts/local-verify-rig.sh`（`scripts/verify-application-url.mjs` を追加した）。

### 未検証区間

**本番で、実際にログインした状態から申し込みフォームのURLを貼るところは未実測である。**
理由: この環境に実Googleアカウントの資格情報が無い。
確かめる手段: ユーザーが本番で自分の企画を開き、「参加申し込みフォームのURL（任意）」へ
フォームのURLを貼って保存し、公開ページに「参加を申し込む」ボタンが出ることを見る。

### 本番反映と、DB適用の機械的な確認（2026-09-13）

PR #62（`48e7edc`）を`main`へマージし、本番へ反映した。ユーザーが
`drizzle/0004_proposal_application_url.sql` を本番Neonへ適用した。
**適用されたことは、ユーザーの報告ではなくこちらで機械的に確かめた**（失敗記録 F-02 への対処）。

| 見たこと | 実測 |
| --- | --- |
| `48e7edc` のVercelデプロイ | `success`（Deployment has completed）＝新しいコードが本番で動いている |
| 企画一覧 | HTTP 200 / 公開中2件 |
| 企画詳細（公開中2件とも） | **HTTP 200**。このページは `application_url` を select するので、**列が無ければ500になる**。200＝列が存在する |
| 「参加を申し込む」ボタン | 出ていない（まだ誰もURLを入れていないので、これが正しい） |
| トップ | HTTP 200 |

**マージ前に確かめる手段は無かった。** プレビュー環境はVercelのSSO保護で外から開けず（302）、
本番の旧コードは `application_url` を読まないため、外から列の有無を観測できない。
そのことをユーザーへ先に伝えたうえで、失敗しても企画詳細が500になるだけで
revertで戻せることを確認してからマージした。

### 途中で起きたこと（次回の申し送り）

- **PR作成直後にCIが1度も起動しなかった。** 押した時点でPRが無く、その後PRを開いても
  GitHub Actionsの実行が作られなかった。**空コミットや閉じ直しでCIを起こすのは禁止**なので
  行わず、状況をユーザーへ伝えた。**枝分かれを解消して押し直したとき（`synchronize`）に
  正常に起動し、`verify` が success になった。**
- **squashマージ由来の枝分かれでマージが競合した**（`405 merge conflicts`）。台帳に記録済みの
  手順（`git reset --hard origin/main` → 差分を当て直す → 再コミット → `--force-with-lease`）で解消した。

## 今回の作業（2026-09-12 / P34 通報の階層下げ・画像添付・権限の統一・導線の実操作検証、Claude Code on the web）

ユーザーの指示は4つ。①通報の操作画面が大きすぎて通報ページに見えるので1階層下げろ、
②画像を添付できるようにしろ、③権限の曖昧を①管理者②登録者③一般の3つに統一しろ、
④**登録者ページと企画ページに一貫性が無い。操作経路をチェックリストにして、順方向・分岐・
逆方向を実操作で検証しろ**（「お前は直接ページに飛んで確認しているから気づいていない」）。

指摘は当たっていた。**実操作で25項目を通したところ、12項目が到達不能だった。**

### ④ まず現状を実測した（修正前）

ローカルにPostgreSQL 16を立てて本番と同じマイグレーションを当て、実Chromiumで
一般・登録者・管理者の3つの立場から画面を順に押した。押せるリンクがあるか、
実際に押して着いたかで判定している。文字があるだけでは合格にしない。

修正前にNGだった12件:

| ID | 起点 | 到達できなかったもの |
| --- | --- | --- |
| G-06 | `/events/[slug]` | トップへ戻れない |
| G-07 | `/events/[slug]` | 「自分も企画を立てる」へ行けない |
| G-08 | `/events/[slug]` | 通報の入力欄が企画本文と同じページに直接置かれている |
| M-01b | `/member/profile` | 規約更新後の再同意画面から会員ページへ戻れない |
| M-04 | `/member` | 履歴へ行けない |
| M-05 | `/member` | 公開中の企画一覧へ行けない |
| M-07 | `/member/proposals/[id]` | 来た道（自分の企画一覧）へ戻れない |
| M-08 | `/member/proposals/[id]` | **公開中の自分の企画の公開ページを見られない** |
| M-11 | `/member/history` | 会員ページへ戻れない |
| M-12 | `/member/proposals/new` | 企画一覧へ戻れない |
| M-13 | `/events/[slug]` | 自分の企画の公開ページから編集画面へ行けない |
| M-14 | 全ページ | 右上のバッジが常に `/member/profile` 固定で、会員ページへ1回で行けない |

### 直したもの

**① 通報を1階層下げた。** `/events/[slug]/report` を新設し、企画詳細ページからは
「管理者へ連絡」ボタン1つだけを出すようにした。詳細ページに通報の入力欄は無くなった。
送信すると企画ページへ戻り、「受け付けました」と出る（以前は `?reported=1` を付けて
戻っていたが、受け取る側の表示が無く、何も起きていないように見えていた）。
通報ページは検索結果に出さない（`robots: noindex`）。

**② 画像を1枚添付できるようにした。** 保存先はNeonのDB（`drizzle/0003_proposal_images.sql`）。
外部のファイル保管サービスを使うと、ユーザーが保管領域を作り鍵を発行して環境変数へ
入れる作業が要るため、それが要らない方法を選んだ。**送信前にブラウザ側で長辺1280pxへ
縮め、JPEGへ再エンコードする**ので、スマートフォンの写真をそのまま選んでよい
（実測: 2314KBのPNG → 22KBのJPEG）。下書きのうちは持ち主と管理者にしか見えない。
配信は `/api/proposals/[id]/image`。

**③ 立場を3つに定めた。** `src/lib/auth/roles.ts` に「管理者・登録者・一般」と、
それぞれができること・必要な手続きを1か所へ置き、`/register` と `/member` に
**同じ表**を出す。いまの自分がどれかも示す。あわせて画面に混ざっていた
「メンバー登録」を「会員登録」へ、「メンバー」を「会員」へ揃えた。

**④ 導線12件をすべて直した。** パンくず（会員ページ / 自分の企画 / この企画）を
会員側の4画面へ入れ、企画詳細から公開ページへ、公開ページから編集へ、双方向で
行き来できるようにした。右上のバッジは、有効な会員なら `/member` へ送る。

### 検証（肯定・否定の両方を実測した）

| 見たこと | 実測 |
| --- | --- |
| 操作経路の検証（一般・登録者・管理者） | **40項目すべてOK**（修正前は25項目中12件NG） |
| 画像の端から端まで | **10項目すべてOK** |
| **否定側**: バッジを元に戻す | M-14 だけがNGに戻る |
| **否定側**: 「管理者へ連絡」とトップへの導線を外す | G-06・G-10 がNGに戻り、その先で止まる |
| **否定側**: `/register` から立場の表を外す | 新しいテスト3件のうち2件が落ちる |

品質ゲート: `lint` 0件、`typecheck` 0件、`NEXT_PUBLIC_NEON_AUTH_ENABLED=true build` 成功、
Playwright **全114件緑**（既存111件＋`tests/standing-guide.spec.ts` 3件）。

### 次のセッションが1コマンドで同じ確認をできるようにした

    bash scripts/local-verify-rig.sh

PostgreSQLを立て、本番と同じマイグレーションを当て、検証用データを入れ、ログイン状態を
作る細工を**一時的に**入れて（終了時に必ず消す。trapで取り消すので途中で失敗しても残らない）、
実ブラウザで操作経路と画像添付を通す。まっさらな状態から通して 全体判定 OK を実測した。

この環境でブラウザを使うときの注意を1つ追記する。**開発サーバーは `localhost` で開くこと。**
`127.0.0.1` で開くと Next の開発サーバーが別オリジン扱いでJSを403にし、画面が組み上がらない
（＝押しても何も起きない）。今回これで1度、実装の不具合と読み違えかけた。

### 本番での実測（2026-09-12、マージ後）

PR #59（`22c8951`）を`main`へマージし、本番へ反映された。**ユーザーがDBの適用を終えたと報告した
あと、こちらで機械的に確かめた**（報告だけを根拠にしていない。失敗記録 F-02 への対処）。

**DBの適用が効いていること**（新しい列を読む経路で判定した。列が無ければ500になる）

| 見たこと | 実測 |
| --- | --- |
| トップ（`image_mime` を読む） | 200 |
| 企画一覧（`image_mime` を読む） | 200 |
| `/api/proposals/<uuid>/image`（3列すべて読む） | **404**（500ではない＝列が存在する） |

**通報の階層下げと導線**（本番に公開中の企画が1件あったので、それで確かめた）

| 見たこと | 実測 |
| --- | --- |
| 企画詳細の通報の入力欄 | **0個**（修正前は2個） |
| 企画詳細の「管理者へ連絡」ボタンと通報ページへのリンク | 各1個 |
| 通報ページ | 200、入力欄2個、企画へ戻る導線あり、検索除け（noindex）あり |
| 企画詳細 → トップ／会員登録 | どちらも到達可 |
| 「参加するのに会員登録は要りません」の明記 | 表示あり |
| `/register` の3つの立場 | 管理者・登録者・一般がすべて表示 |

**通報の送信そのものは、本番では試していない。** 送ると本物の通報記録が本番のDBへ残るためである。
送信経路はローカルの検証装置で通してある（G-14・G-15）。

### 画像の端から端まで、本番で通った（2026-09-12）

ユーザーが本番でログインし、自分の企画へ写真を1枚添付した。**報告だけを根拠にせず、
こちらで配信された実物を取得して確かめた。**

| 見たこと | 実測 |
| --- | --- |
| 画像が未ログインの人にも配信される | HTTP **200** / `content-type: image/jpeg` / `content-length: 89357`（実バイト数も一致） |
| ブラウザ側の縮小が効いている | **1280 × 853 px**（JPEGのSOFセグメントから読んだ。設計どおり長辺が1280） |
| 企画詳細ページ | 出ている |
| 企画一覧・トップページ | どちらも出ている |
| キャッシュの扱い | `cache-control: public, max-age=60`（公開中の企画なので公開キャッシュで正しい） |

**これで「ログイン → 画像を選ぶ → ブラウザ側で縮小 → 保存 → 公開ページで配信」の全区間が
本番で通ったことになる。** あわせて、ログイン・企画編集画面・保存の経路自体も本番で
動いていることが確かめられた（画像が保存できた以上、その手前は通っている）。

### 未検証区間

**会員側の画面（`/member` 配下）の導線が、本番でも期待どおりかは未実測である。**
具体的には、パンくず（会員ページ / 自分の企画 / この企画）、履歴・公開一覧への導線、
右上バッジの行き先。ローカルの検証装置では40項目すべてOKだが、本番では見ていない。
理由: この環境に実Googleアカウントの資格情報が無く、`/member` 配下は未ログインだと
`/member/profile` へ転送されるため外からは見えない。
確かめる手段: ユーザーが本番でログイン中に `https://aiueo-lp.vercel.app/member/proposals` を開き、
上部に「会員ページ / 自分の企画」と出ていること、企画を開くと「公開ページを見る」が出ることを見る。

### 直していないもの（意図的）

- 管理画面の企画編集フォームには画像の欄を足していない。管理者がDBの値を直接書き換える
  ための画面で、画像だけ別の操作になるため。管理者が編集しても画像は消えない（触らない）。
- 「企画者」と「主催者」の言い分けは残した。**権限ではなく役割の名前**で、公開ページで
  参加者が見るのは「主催者」、会員・管理画面で操作するのは「企画者」である。統一すると
  かえって意味が落ちる。
...
### 解消済み: P37（参加人数）を安全に入れ直した（2026-09-15）

1度目（PR #64 `4b0e9a7`）は `drizzle/0005` の適用を待たずにマージし、本番を2分30秒落とした（F-11）。
2度目は次の順番を守り、本番反映まで完了した。

1. P38の窓口（`/api/health/schema`）へ2列を `pending` で登録して**先に出す**
2. ユーザーが `drizzle/0005_proposal_headcount.sql` を本番Neonへ適用
3. **こちらで機械的に確認**（`pendingMissing` が `[]`、2列とも `present: true`）
4. `git revert af03509`（revertのrevert）でコードを戻してマージ
5. 窓口の2行を `required` へ移動

**今後、新しい列を足すときは必ずこの順番で行う。**

残る未検証区間: 本番で実ログインした状態からの人数入力。
確かめる手段: ユーザーが本番で自分の企画に「定員」と「参加人数」を入れて保存し、
`https://aiueo-lp.vercel.app/events` のカードに「参加人数 3 / 10人（残り7）」と出ることを見る。
...
- **利用者の操作経路と画像添付は `bash scripts/local-verify-rig.sh` で確かめられる。** PostgreSQLを立て、本番と同じマイグレーションを当て、ログイン状態を作る細工を一時的に入れて（終了時に必ず消す）、実ブラウザで通す。**画面を直したら、まずこれを走らせること。** 「直接URLを開いて表示を見る」だけでは、押せるリンクが無いことに気づけない（2026-09-12、実際に12件の断絶を見落としていた）。
- **この環境で開発サーバーを見るときは `localhost` を使う。** `127.0.0.1` で開くとNextの開発サーバーが別オリジン扱いでJSを403にし、画面が組み上がらない。押しても何も起きなくなるので、実装の不具合と読み違えやすい。
- **`drizzle/0003_proposal_images.sql` は本番Neonへ適用済み**（2026-09-12、ユーザーがVercelのQuery画面から実行）。**適用されたことは、こちらで機械的に確かめた**（ユーザーの報告だけを根拠にしていない）。本番反映後に、新しい列を読む3経路がすべて正常応答することを実測した: トップ200・企画一覧200（どちらも `image_mime` を読む）・`/api/proposals/<uuid>/image` が404（列が無ければ500になる）。**全体を1つの `do` ブロックにしてあるので、Vercelの画面へ1回貼るだけで済む。** 何度流しても結果は変わらない。
- **企画の行を `select *` / `returning *` で取って監査ログへ入れるときは、必ず `withoutImageData()` を通す。** 通さないと `image_data` が `{"type":"Buffer","data":[...]}` という元画像より大きなJSONになって、保存のたびに版履歴と監査ログの2か所へ積み上がる。**2026-09-13、この漏れが `src/app/admin/actions.ts` の `recordAdminChange` に残っていた**（P36）。実測で 1操作あたり 1,433,122 バイト積まれていた。直した後は 815 バイト。**`profiles` と `reports` を渡す2か所は画像の列を持たないので通さなくてよい。**
- **`drizzle/0004_proposal_application_url.sql` は本番Neonへ適用済み**（2026-09-13、ユーザーがVercelのQuery画面から実行）。**適用されたことは、こちらで機械的に確かめた**（`48e7edc` のVercelデプロイ成功＋企画詳細ページ 200。このページは `application_url` を select するので、列が無ければ500になる）。**1つの `do` ブロックなので、画面へ1回貼るだけで済む。**
- **プレビュー環境はVercelのSSO保護で外から開けない**（302 → `vercel.com/sso-api`）。DBのマイグレーションが要る変更は、**マージ前に外から確かめる手段が無い**。マージ後すぐ確かめること。失敗しても該当ページが500になるだけで revert で戻せる。この制約はユーザーへ先に伝えてからマージする。
- **PRを作った直後にCIが起動しないことがある。** 2026-09-13、押した時点でPRが無く、その後PRを開いてもGitHub Actionsの実行が作られなかった（赤ではなく「未報告」）。**空コミットや閉じ直しでCIを起こすのは禁止。** 次にコードを押した時点（`synchronize`）で正常に起動する。
