...
- 最新の実装コミット: `622b536 feat: トップページの「進行中の企画」をDBの公開企画に接続する`（ブランチ`claude/checkin-6hrtds`、PR作成・マージ待ち）
...
設計図の不足#2（トップページの「進行中の企画」がDB未接続）に対応。ブランチ`claude/checkin-6hrtds`、コミット`622b536`。
...
2026-09-06に対応済み（ブランチ`claude/checkin-6hrtds`、コミット`622b536`、上の「今回の作業」参照）。`/events`と同じDBデータを表示し、0件時はセクション非表示、タグ絞り込みボタンは削除、画像は`mock.ts`の既存素材を流用。**まだ本番マージ・本番の実企画データでの見た目確認はしていない。**
