...
- 最新の実装コミット: `22c8951 feat: 通報を1階層下げ、画像添付を追加し、立場を3つへ統一し、導線の断絶12件を直す(P34) (#59)`（`main`へマージ済み・**本番反映済み**）
...
**`drizzle/0004_proposal_application_url.sql` は本番Neonへ未適用である。** 適用するまで、
この機能を含むコードを本番へ出すと企画の保存が失敗する（列が無いため）。
**マージより先に適用すること。**
...
- **企画の行を `select *` / `returning *` で取って監査ログへ入れるときは、必ず `withoutImageData()` を通す。** 通さないと `image_data` が `{"type":"Buffer","data":[...]}` という元画像より大きなJSONになって、保存のたびに版履歴と監査ログの2か所へ積み上がる。
