...
> **廃止（2026-09-06決定、恒久）**: 本ドキュメントはSupabase RLS/RPC前提で書かれたが、基盤をNeonへ移行した後もRLSは実装されず、2026-09-06にRLSを今後も採用しない方針を正式決定した（`IMPLEMENTATION_PLAN.md`Gate 1決定状況「RLS不採用」を参照）。以下の内容は実行しない。認可の受入テストはサーバーDAL層のallow/deny検証として別途行う。履歴として残すため削除はしない。
