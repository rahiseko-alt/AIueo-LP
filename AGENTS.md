...
2026-09-06、`IMPLEMENTATION_PLAN.md`のP3〜P6の状態欄が「DB未適用」「Edge Function未デプロイ」等の古い文言のまま残り、実態（DB適用済み・メール送信コード自体が無い等）と食い違っていた。前回のセッションの文言をコードの再確認なしにそのまま転記したのが原因である。

**状態欄を書く・引用するときは、必ずコード（`src/` `drizzle/` `supabase/`）を確認してから書く。** 前回の文言が正しいという前提を置かない。`.claude/hooks/session-start.sh`が、フェーズ表の各行の最終更新日とコードの最終更新日を比較し、コードの方が新しい行を再確認候補として開始時に出す。これは誤りの証明ではなく注意なので、出た行を無条件で書き換えるのではなく、実際にコードを見て確認したうえで直すか、現状のままでよいと判断したかを進捗記録に残すこと。
...
## 6. ユーザーへの説明は非エンジニア向けに徹する

このプロジェクトのユーザーはエンジニアではない。とくに Google Cloud、Neon、Vercel などの
**外部サービスの設定作業を案内するとき**は、次を守ること。

1. **画面名・ボタン名・入力値をそのまま示す。** 「OAuthクライアントを設定してください」ではなく、
   「左メニューの『クライアント』→『OAuth クライアントを作成』→ 種類は『ウェブアプリケーション』」
   のように、ユーザーが目で探して押せる粒度まで下ろす。入力欄には実際に入れる文字列を書く。
2. **専門用語を裸で出さない。** 出すときは同じ行で言い換える（例:「リダイレクトURI（ログイン後に
   戻ってくる先のURL）」）。
3. **1回の返信で頼む操作は1区切りにする。** 手順を最後まで並べるのではなく、その画面で終わる分だけ
   示し、「次はこの値を貼り戻してください」と、ユーザーが返すものを明示して終える。
4. **推測で値を埋めさせない。** こちらが確定値を知らない項目は `【曖昧】` を付け、**どこの画面で
   取得できるか**を示す。それらしい値をでっち上げない。
5. **エラー画面や想定外の表示を見せられたら、まず成否を判定して先に言う。** 「成功しています」
   「失敗しています」を最初の1文に置き、そのうえで次の操作を示す。ユーザーに状況判断をさせない。
6. **ユーザー側の外部作業と、こちら側のコード作業を毎回はっきり分ける。** どちらの手番かを明示する。

この節は 2026-09-09、Google認証（P15）の設定案内でユーザーから「非エンジニア向けのナビに徹しろ」と
指示を受けて追加した。
